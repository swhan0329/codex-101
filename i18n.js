...
const translations = {
    ko: {
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, `developers.openai.com/blog`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. `Mastering Codex Remote for engineering`은 Remote connections 문서의 phone-as-control-center 운영 패턴을 보강하고, private MCP server 글은 Secure MCP Tunnel과 Platform/API 보안 연결 맥락으로만 둡니다. Simplex customer story는 ChatGPT Enterprise와 Codex를 쓰는 enterprise adoption 및 AI-first delivery 사례로, `Dreaming`은 ChatGPT memory 제품 업데이트로 분리했습니다. 둘 다 새 일반 Codex 모델 순서, 일반 사용자 플랜, local app 설정 키, credential/permission 변화로 단정하지 않았습니다.",
        ...
        changelog_item_5: "OpenAI Developers/blog와 openai.com/index를 확인했고, Codex Remote, private MCP server/Secure MCP Tunnel, Simplex, ChatGPT Dreaming은 각각 운영, Platform/API, enterprise, ChatGPT memory 맥락으로만 분리했습니다.",
        ...
        field_tips_intro: "2026-07-05 저장된 social research note를 baseline으로 다시 확인했습니다. Chrome plugin 연결을 시도했지만 Chrome extension backend가 unavailable이고 Codex in-app browser만 노출되어 X/Threads/LinkedIn을 사용자 Chrome 세션으로 안정적으로 확인하지 못했습니다. 검증되지 않은 소셜 claim은 추가하지 않았고, 제품 상태와 활성화 방법은 공식 Codex 문서, Developers 블로그, OpenAI 발표로만 대조했습니다.",
    ...
    en: {
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `developers.openai.com/blog`, `openai.com/index`, and `openai.com/codex` together. `Mastering Codex Remote for engineering` reinforces the Remote connections phone-as-control-center pattern, while the private MCP server post stays in the Secure MCP Tunnel and Platform/API security lane. The Simplex customer story is recorded as enterprise adoption and AI-first delivery context for ChatGPT Enterprise plus Codex, and `Dreaming` is separated as a ChatGPT memory update. None of these are general Codex model-order, consumer-plan, local app config, credential, or permission changes.",
        ...
        changelog_item_5: "Rechecked OpenAI Developers/blog and openai.com/index; Codex Remote, private MCP server/Secure MCP Tunnel, Simplex, and ChatGPT Dreaming were added only in their proper operating, Platform/API, enterprise, or ChatGPT-memory lanes.",
        ...
        field_tips_intro: "On July 5, 2026, I reused the saved people/social research note as the baseline. Chrome plugin connection was attempted, but the Chrome extension backend was unavailable and only the Codex in-app browser was exposed, so X, Threads, and LinkedIn could not be checked reliably through the user's Chrome session. No unverified social claim was added; product status and enablement steps are checked against official Codex docs, Developers blog posts, and OpenAI announcements.",
