...
const translations = {
    ko: {
        ...
        overview_fact_5_desc: "`Introducing the Codex app`와 2026년 4월 16일 `Codex for (almost) everything` 발표, 현재 `app/features`·`app/automations`·`app/settings`를 다시 대조해 worktrees, built-in Git, integrated terminal, skills, Automations, thread automations, chats, in-app browser, artifact preview, image generation뿐 아니라 background computer use, memories, context-aware suggestions, 더 넓어진 plugin 생태계, multiple terminal tabs, SSH devbox, summary pane 흐름까지 앞쪽에서 이해할 수 있게 재배치했습니다. Settings > Profile에서 lifetime tokens, peak tokens, streaks, longest task, token activity를 볼 수 있다는 점도 App 운영 흐름에 추가했습니다. 2026년 5월 18일 Dell 파트너십 발표는 하이브리드·온프레미스 enterprise 배포 방향으로만 반영했습니다.",
        ...
        changelog_item_4: "Codex App Settings > Profile에서 lifetime tokens, peak tokens, streaks, longest task, token activity를 확인할 수 있다는 공식 설정 문서를 추가했습니다.",
        changelog_item_5: "Docs MCP, best practices, config, app/settings, OpenAI API changelog/deprecations, API 모델 문서, Windows sandbox 발표, Dell/Gartner enterprise 발표, election safeguards의 Codex Security/provenance 맥락을 다시 확인했습니다.",
        changelog_item_6: "Chrome에서 X 검색은 로딩/뉴스 사이드바만 확인됐고, 추가 소셜 claim은 문서에 넣지 않았습니다. Threads/LinkedIn은 로그인·정책 제한 가능성이 있어 공식 문서 근거만 반영했습니다.",
    ...
    en: {
        ...
        changelog_item_4: "Added the official Codex App Settings > Profile stats: lifetime tokens, peak tokens, streaks, longest task, and token activity.",
        changelog_item_5: "Rechecked Docs MCP, best practices, config, app/settings, the OpenAI API changelog/deprecations, API model docs, the Windows sandbox post, Dell/Gartner enterprise announcements, and election-safeguards Codex Security/provenance context.",
        changelog_item_6: "Chrome/X only exposed loading and news/sidebar surfaces during the social check, so no unverified social claim was added; Threads/LinkedIn remain treated as access-limited for this run.",
        ...
        s13_c1_d: "Start by fixing the defaults that most affect consistency: model choice, model_provider, review_model, reasoning effort, sandbox mode, approval policy, web_search, profiles, and MCP setup. Teams using the AWS limited-preview Bedrock path should treat provider configuration as part of the rollout standard. For native Windows work, review `windows.sandbox` and `windows.sandbox_private_desktop`; manage remote connection through App Settings > Connections and workspace Remote Control permissions.",
