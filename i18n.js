...
const translations = {
    ko: {
        ...
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog, Windows sandbox 발표를 다시 확인하고 Platform/API 신규 항목은 Codex 기본 사용법과 분리했습니다.",
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn을 확인했습니다. X는 빈 본문, LinkedIn은 내비게이션만 확인됐고, Threads에서 읽힌 팁은 공식 문서로 검증 가능한 출력 계약·툴 규칙·완료 기준 조언만 기존 prompting guidance와 연결했습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome 연결이 성공했습니다. X는 읽을 수 있는 본문이 비어 있었고 LinkedIn은 내비게이션만 노출됐습니다. Threads에서는 일부 Codex 게시물을 읽을 수 있었지만, 공식 문서와 맞는 출력 계약·도구 사용 규칙·완료 기준 조언만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 19 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. The May 19 check also records that Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 gets a usage boost through the same date. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, the OpenAI API changelog, and the Windows sandbox post; Platform/API updates stay separate from core Codex usage.",
        changelog_item_6: "The Chrome plugin checked X, Threads, and LinkedIn. X returned blank readable text, LinkedIn exposed navigation only, and the readable Threads tips were folded only into already official guidance on output contracts, tool rules, and done criteria.",
        ...
        s18_field_credit: "Chrome connected successfully in this run. X returned blank readable content, LinkedIn exposed navigation only, and Threads returned readable Codex posts. I did not add unverified claims; I only tied the official-doc-aligned reminder about output contracts, tool-use rules, and done criteria back to the existing prompting guidance.",
