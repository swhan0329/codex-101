...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 7월 1일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 인증 Codex 기준 deprecated로 분리했고, 현재 모델 선택지와 API availability는 범위가 다를 수 있으므로 Codex product model picker, ChatGPT 일반 모델 retirement, API 모델 제공 여부를 따로 읽어야 합니다.",
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 6월 24일 AWS 발표는 Commercial/GovCloud AWS, AWS-native security/governance/procurement/billing, Codex on Amazon Bedrock을 다루므로 enterprise/provider 배포 맥락으로만 설명했습니다. Workspace Agents는 Codex-powered라고 설명되지만 ChatGPT 팀 계정용 shared cloud agent preview이고, 발표문은 Codex app support를 coming soon으로 둡니다. 2026년 6월 26일 GPT-5.6 Sol preview는 일부 신뢰 파트너/조직 preview와 broader availability coming soon으로 분리했습니다. 2026년 6월 28일 HP partnership는 enterprise adoption 사례로만 분리했고, 6월 29일 EU workforce transition report와 6월 30일 global ChatGPT adoption, GeneBench-Pro, core-dump debugging/epidemiology 글은 research, benchmark, 또는 technical operating context로만 기록했습니다. Daybreak/Patch the Planet/Codex-maxxing은 계속 보안/장기작업 운영 맥락이며, 새 일반 Codex 모델 순서, 일반 사용자 플랜, local app 설정 키, credential/permission 변화로 단정하지 않았습니다.",
        overview_update_6_title: "Docs MCP·API changelog·설정 기준 재점검",
        overview_update_6_desc: "Docs MCP, config reference, app/settings를 다시 확인해 project-scoped `.codex/config.toml`이 trusted project에서만 로드되고 machine-local provider/auth/profile/telemetry 키는 user-level config에 둬야 한다는 제한, shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `plan_mode_reasoning_effort`, `approval_policy.granular`, `approvals_reviewer`, `default_permissions`, permissions profile, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, hooks/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. 오늘은 legacy `[profiles.*]`와 `profile = \"...\"` 예시를 현재 문서의 `~/.codex/<profile>.config.toml` 방식으로 고치고, feature flag 표에 남아 있는 `features.prevent_idle_sleep`/`features.network_proxy`는 permissions profile과 managed policy 맥락에서 조심해서 읽도록 정리했습니다. Sites는 `.openai/hosting.json`에 project linkage와 D1/R2 binding 이름을 저장하지만 runtime secret은 Sites panel에서 관리해야 한다는 식으로 분리했습니다.",
        ...
        overview_fact_6_desc: "`review_model`, `approval_policy.granular`, `approvals_reviewer`, top-level `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, `service_tier`, `personality`, `default_permissions`, `windows.sandbox`, `windows.sandbox_private_desktop`, `model_instructions_file`, `background_terminal_max_timeout`, `memories.disable_on_external_context`, `sqlite_home`, TUI 설정, protected paths, JSON schema, app connector 권한, `features.fast_mode`, permissions profile, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, OpenTelemetry export, plugin 배포 계층, Bedrock provider 설정, 로컬 OS 샌드박스와 cloud split runtime 설명을 함께 최신 기준으로 맞췄습니다. 특히 project-scoped config는 trusted project에서만 로드되고 provider/auth/profile/telemetry 라우팅 키는 user-level config에 둬야 합니다. profile은 더 이상 `[profiles.fast]` 같은 inline table이 아니라 `~/.codex/fast.config.toml` 같은 별도 profile file로 관리합니다. `experimental_instructions_file`은 `model_instructions_file`로 바꾸라는 deprecation 메모도 추가했습니다. OpenAI API changelog와 deprecations 문서는 Platform/API 개발자용 흐름으로 따로 읽어야 합니다. GPT-5.5는 API 최신 모델 가이드에서도 유지되며, 기본 reasoning effort medium과 image input detail 기본값 변화는 직접 API 앱 안내에만 반영했습니다.",
        ...
        changelog_item_3: "Quickstart, App/IDE/CLI/Cloud, Remote connections, Windows, Sites, config, MCP/hooks/plugins/skills 문서를 다시 훑었고 기능 활성화 절차 변화는 없었습니다.",
        changelog_item_4: "공식 use-case catalog 60개를 다시 감사했고, promptEn/promptKo 누락 0, official prompt mismatch 0, multiline 한국어 요약 회귀 0을 확인했습니다.",
        changelog_item_5: "OpenAI Developers MCP와 openai.com/index를 확인했고, 6월 30일 ChatGPT adoption, GeneBench-Pro, core-dump debugging 글은 연구/벤치마크/운영 사례로만 분리했습니다.",
        changelog_item_6: "저장된 social research note를 baseline으로 재확인했고, Chrome social 표면은 X 빈 body, Threads 검색 shell, LinkedIn blank tab이라 새 Codex 팁은 승격하지 않았습니다.",
        ...
        s18_field_credit: "이번 실행의 Chrome 소셜 확인에서는 X가 빈 body를 반환하고 Threads가 검색 shell만 보여주며 LinkedIn이 blank tab으로 떨어졌습니다. 소셜 글은 현장 신호로만 다루고, AGENTS/config 기반 재현성, permissions 확인, OpenAI Developers plugin 활용처럼 공식 문서와 맞는 팁만 유지합니다.",
        ...
        field_tips_intro: "2026-07-01 저장된 social research note를 baseline으로 다시 확인했습니다. Chrome에서 X는 빈 body, Threads는 검색 shell만 반환했고 LinkedIn은 blank tab으로 떨어져 검증되지 않은 소셜 claim은 추가하지 않았습니다. 제품 상태와 활성화 방법은 공식 Codex 문서와 OpenAI 발표로 대조했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The July 1 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and that `gpt-5.4-mini` and `gpt-5.3-codex-spark` keep their fast local/subagent and Pro research-preview roles. `gpt-5.2` and `gpt-5.3-codex` are now treated as deprecated for ChatGPT-authenticated Codex rather than fallback recommendations. Current Codex model-picker guidance can differ from API availability, so the guide keeps Codex product choices, ChatGPT model retirement, and API model availability in separate lanes.",
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The June 24 AWS announcement covers Commercial/GovCloud AWS, AWS-native security/governance/procurement/billing, and Codex on Amazon Bedrock, so the guide treats it as enterprise/provider deployment context. Workspace Agents is described as Codex-powered, but it is a shared cloud-agent research preview for ChatGPT team workspaces and says Codex app support is coming soon. The June 26 GPT-5.6 Sol preview is separated as limited partner/organization preview plus broader availability coming soon. The June 28 HP partnership remains enterprise adoption context, while the June 29 EU workforce transition report plus the June 30 global ChatGPT adoption, GeneBench-Pro, and core-dump debugging/epidemiology posts are research, benchmark, or technical operating context only. Daybreak, Patch the Planet, and Codex-maxxing remain security and long-running-work operating context, not general Codex model-order, consumer-plan, local app config, credential, or permission changes.",
        overview_update_6_title: "Docs MCP, API changelog, and config rechecked",
        overview_update_6_desc: "Docs MCP, the config reference, and app/settings were rechecked around trusted project-scoped `.codex/config.toml`, the user-level requirement for machine-local provider/auth/profile/telemetry routing keys, shared CLI/IDE setup, the AGENTS.md steering snippet, `review_model`, `plan_mode_reasoning_effort`, granular approvals, `approvals_reviewer`, `default_permissions`, permissions profiles, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, and hooks/plugin/skill controls. This run corrects legacy profile examples to the current `~/.codex/<profile>.config.toml` file pattern and keeps `features.prevent_idle_sleep` / `features.network_proxy` framed through permissions profiles and managed-policy cautions because they remain in the current feature-flag table. Sites is now clearer too: `.openai/hosting.json` stores project linkage and D1/R2 binding names, while runtime secrets are managed in the Sites panel.",
        ...
        changelog_item_3: "Rechecked Quickstart, App/IDE/CLI/Cloud, Remote connections, Windows, Sites, config, MCP, hooks, plugins, and skills; no activation-flow change was needed.",
        changelog_item_4: "Audited all 60 official use cases again and confirmed 0 missing promptEn, 0 missing promptKo, 0 official prompt mismatches, and 0 multiline Korean summary regressions.",
        changelog_item_5: "Rechecked OpenAI Developers MCP and openai.com/index; the June 30 ChatGPT adoption, GeneBench-Pro, and core-dump debugging posts are separated as research, benchmark, or operating context only.",
        changelog_item_6: "Reused the saved social research note as the baseline; X returned an empty body, Threads exposed only search shell content, and LinkedIn fell back to a blank tab.",
        ...
        s18_field_credit: "This run used Chrome to recheck the targeted X, Threads, and LinkedIn surfaces; X returned an empty body, Threads exposed search shell content, and LinkedIn fell back to a blank tab. Social posts stay as field signals, and the guide keeps only tips that fit official docs, such as AGENTS/config reproducibility, permission checks, and OpenAI Developers documentation workflows.",
        ...
        field_tips_intro: "On July 1, 2026, I reused the saved people/social research note as the baseline. X returned an empty body, Threads exposed only search shell content, and LinkedIn fell back to a blank tab. Social posts remain field signals only; product status and enablement steps are checked against official Codex docs and OpenAI announcements.",
