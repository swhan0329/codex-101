...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 6월 6일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 중요한 변화는 현재 models 문서가 `gpt-5.2`와 `gpt-5.3-codex`를 ChatGPT 로그인 기준 Codex에서는 deprecated라고 명시한다는 점입니다. API-key authentication을 쓰는 workflow는 API models page에서 별도로 확인해야 하며, 임의 provider에서 Chat Completions API를 쓰는 Codex 경로는 future release에서 제거될 예정인 deprecated 지원입니다.",
        overview_update_2_title: "요금제·크레딧 구조 재정리",
        overview_update_2_desc: "개인 플랜 한도뿐 아니라 `GPT-5.5` 사용량 표, `Free $0`·`Go $8`·`Plus $20`·`Pro from $100`, Plus의 web/CLI/IDE/iOS 접근, Business·Enterprise의 credits 설명, 그리고 현재 `codex/pricing` 문서의 Plus / Pro 5x / Pro 20x / Business / API Key 구성을 다시 맞췄습니다. 어제까지 남아 있던 종료된 Pro promo 문구는 제거하고, 현재 문서처럼 Pro를 Plus 대비 5x 또는 20x higher rate limits로 설명했습니다. 가격표에는 Sites가 Business/Enterprise에서 available, Plus/Pro/API Key에서는 unavailable로 표시되고, Sites FAQ는 preview 기간 동안 무료라고 설명합니다. ChatGPT for Excel 같은 agentic feature도 유효 시점 이후 Codex usage limits를 공유할 수 있으므로, usage dashboard와 `/status`를 함께 확인하라고 정리했습니다.",
        ...
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. `codex/pricing`은 오늘도 Plus / Pro 5x / Pro 20x / Business / API Key 표를 유지하고, Plus 로컬 메시지 범위에는 아직 `gpt-5.3-codex` 30-150이 남아 있습니다. 하지만 models 문서는 이 모델을 ChatGPT 로그인 기준 deprecated로 표시하므로, 가격 한도와 추천 모델 상태는 서로 다른 범위로 읽어야 합니다. Plus는 web, CLI, IDE extension, iOS 접근을 함께 표시합니다. Sites는 현재 Business/Enterprise preview로 표시되고, pricing FAQ는 preview 기간 동안 무료라고 설명합니다. 다만 API-key 관점에서는 `codex/models`와 `codex/pricing`의 표현이 다르므로, 실무 문서는 사용량 표, token credit 표, usage dashboard, App `Settings > Profile`의 lifetime tokens/token activity, 실제 모델 선택기, API 모델 가격표를 섞지 말고 함께 확인하도록 고쳤습니다.",
        ...
        changelog_item_1: "`gpt-5.5` 우선 추천은 유지하고, `gpt-5.2`와 `gpt-5.3-codex`가 ChatGPT 인증 Codex에서 deprecated로 표시된 점을 반영했습니다.",
        changelog_item_2: "`codex/pricing`의 `gpt-5.3-codex` 한도 표기는 가격 범위로 남기되, models 문서의 현재 추천/지원 상태와 분리했습니다.",
        changelog_item_3: "ChatGPT Active sessions는 Codex/API Platform session을 보여줄 수 있지만 Codex CLI session은 관리하지 않는다는 보안 메모를 추가했습니다.",
        changelog_item_4: "OpenAI API latest-model, API changelog, Apps SDK changelog를 OpenAI Developers MCP로 다시 확인했고 June 5 이후 Codex-101 핵심 API 변경은 발견하지 못했습니다.",
        changelog_item_5: "Remote connections, Sites, Bedrock, Windows Computer Use, macOS certificate update 문구는 이전 공식 상태가 유지되는지 재확인했습니다.",
        changelog_item_6: "Chrome 소셜 확인은 X 빈 화면, Threads 타임아웃, LinkedIn 내비게이션만 확인되어 social-only claim을 추가하지 않았습니다.",
        ...
        s18_field_credit: "이번 실행의 Chrome 소셜 확인에서는 X가 빈 화면으로 렌더링됐고, Threads는 타임아웃됐으며, LinkedIn은 내비게이션만 노출했습니다. 검증되지 않은 소셜 claim은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The June 6 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and that `gpt-5.4-mini` and `gpt-5.3-codex-spark` keep their fast local/subagent and Pro research-preview roles. The important change is that the current models page now lists `gpt-5.2` and `gpt-5.3-codex` as deprecated in Codex when you sign in with ChatGPT. API-key workflows should verify API models separately, and the arbitrary-provider Chat Completions path remains a deprecated Codex support path that will be removed in a future release.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. Stale Pro promo language was removed; the current docs simply describe Pro as 5x or 20x higher rate limits than Plus. Pricing still lists Sites as available for Business/Enterprise and unavailable for Plus/Pro/API Key, while the Sites FAQ says it is free during preview. The guide also calls out that agentic features such as ChatGPT for Excel can share Codex usage limits once pricing for those features is effective.",
        ...
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / API Key tables, and its Plus local-message ranges still include `gpt-5.3-codex` 30-150. Because the models page now deprecates that model for ChatGPT sign-in, the guide separates pricing limits from current model recommendations. Plus explicitly includes web, CLI, IDE extension, and iOS access. Sites is listed as a Business/Enterprise preview, and the pricing FAQ says it is free during preview. The important split is that API-key availability is described differently across `codex/models` and `codex/pricing`; direct OpenAI API access to `gpt-5.5` and `gpt-5.5-pro` belongs to the separate API Models/Pricing docs. The guide tells readers to check usage tables, token credit rates, the usage dashboard, App Settings > Profile lifetime-token and token-activity stats, the actual picker, and API model pricing without collapsing them into one bucket.",
        ...
        changelog_item_1: "Kept `gpt-5.5` as the top recommendation and reflected that `gpt-5.2` plus `gpt-5.3-codex` are now deprecated for ChatGPT-authenticated Codex.",
        changelog_item_2: "Kept `codex/pricing` usage-limit wording separate from current `codex/models` recommendations where `gpt-5.3-codex` still appears in pricing tables.",
        changelog_item_3: "Added ChatGPT Active Sessions as security context: it may show Codex/API Platform sessions, but does not manage Codex CLI sessions.",
        changelog_item_4: "Rechecked OpenAI API latest-model, API changelog, and Apps SDK changelog through OpenAI Developers MCP; no June 5+ Codex-101-critical API change surfaced.",
        changelog_item_5: "Revalidated prior Remote connections, Sites, Bedrock, Windows Computer Use, and macOS certificate-update guidance without changing their status.",
        changelog_item_6: "Chrome social verification returned a blank X page, Threads timeout, and LinkedIn navigation only; no social-only claim was added.",
        ...
        s18_field_credit: "Chrome social verification returned a blank X page, a Threads timeout, and LinkedIn navigation only. No social-only claim was added.",
