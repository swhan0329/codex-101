...
const translations = {
    ko: {
        ...
        changelog_item_5: "Docs MCP, config, app/settings, OpenAI API changelog/deprecations, Apps SDK changelog, API 모델 문서, Windows sandbox 발표, Dell/Gartner enterprise 발표를 다시 확인했습니다.",
        changelog_item_6: "Chrome 소셜 확인에서 X는 읽을 수 없었고 Threads/LinkedIn은 커뮤니티 글만 확인됐습니다. 공식 문서로 검증된 Appshots/Profile/Windows Computer Use claim만 반영했습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome에서 X 본문이 읽히지 않았고, Threads에는 Appshots와 oh-my-codex 관련 커뮤니티 글, LinkedIn에는 Profile/token usage 관련 글이 보였습니다. 검증되지 않은 소셜 주장은 추가하지 않았고, 공식 릴리스 노트와 문서로 재확인된 Appshots, Profile, Windows Computer Use, 권한 확인, 출력 계약, 도구 사용 규칙, 완료 기준, OpenAI Developers plugin 활용 메모만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        changelog_item_5: "Rechecked Docs MCP, config, app/settings, the OpenAI API changelog/deprecations, Apps SDK changelog, API model docs, the Windows sandbox post, and Dell/Gartner enterprise announcements.",
        changelog_item_6: "Chrome social verification left X unreadable and found only community Threads/LinkedIn posts, so only officially corroborated Appshots/Profile/Windows Computer Use claims were added.",
        ...
        s18_field_credit: "Social verification in Chrome left X unreadable, while Threads showed community discussion around Appshots/oh-my-codex and LinkedIn showed Profile/token-usage discussion. I did not add unverified social claims; only officially corroborated Appshots, Profile, Windows Computer Use, permissions, output contracts, tool-use rules, done criteria, and OpenAI Developers plugin notes were tied back to the guide.",
