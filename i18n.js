...
const translations = {
    ko: {
        ...
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog/deprecations, API 모델 문서, Windows sandbox 발표, Dell/Gartner enterprise 발표, election safeguards의 Codex Security/provenance 맥락을 다시 확인했습니다. 현재 config reference에 `plan_mode_reasoning_effort`가 보이므로 Plan mode 전용 reasoning override로 되돌려 설명했습니다.",
        changelog_item_6: "소셜 확인에서 X는 로그인 리다이렉트, Threads는 가입/로그인 모달, LinkedIn은 로그인 화면에 막혔습니다. 검증되지 않은 소셜 claim은 추가하지 않았습니다.",
        ...
        s18_field_credit: "이번 실행에서는 X가 로그인 리다이렉트로 이동했고, Threads는 가입/로그인 모달, LinkedIn은 로그인 화면에 막혀 읽을 수 있는 현재 Codex tip을 확인하지 못했습니다. 검증되지 않은 주장은 추가하지 않았고, 공식 문서와 맞는 권한 확인, 출력 계약, 도구 사용 규칙, 완료 기준, OpenAI Developers plugin 활용 메모만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, the OpenAI API changelog/deprecations, API model docs, the Windows sandbox post, Dell/Gartner enterprise announcements, and election-safeguards Codex Security/provenance context; `plan_mode_reasoning_effort` is back as a current Plan mode override.",
        changelog_item_6: "Social verification hit access gates: X redirected to login, Threads showed a signup/login modal, and LinkedIn showed login. No unverified social claim was added.",
        ...
        s18_field_credit: "Social verification hit access gates in this run: X redirected to login, Threads showed a signup/login modal, and LinkedIn showed login. I did not add unverified claims; I only tied official-doc-aligned notes on permissions, output contracts, tool-use rules, done criteria, and the OpenAI Developers plugin back to existing guidance.",
