...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 6월 4일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex`의 Cloud/code review 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 특히 오늘도 `codex/models`가 `gpt-5.5`의 API Access를 true로 표시하는 반면 `codex/pricing`의 API Key 표는 아직 Not available로 두는 차이를 분리해 설명했습니다. 추가로 임의 provider에서 Chat Completions API를 쓰는 Codex 경로는 future release에서 제거될 예정인 deprecated 지원이라는 점을 모델 섹션에 반영했습니다.",
        ...
        changelog_item_2: "가격 설명에서 종료된 Pro 프로모션 문구를 제거하고, 현재 Pro 5x/20x 구조, Go/iOS 표기, ChatGPT for Excel 공유 한도, credit 적용 범위를 맞췄습니다.",
        ...
        changelog_item_4: "May 13 보안 공지 기준 macOS 사용자는 2026년 6월 12일까지 Codex App/CLI를 공식 경로로 업데이트해야 한다는 인증서 교체 주의점을 추가했습니다.",
        changelog_item_5: "Config reference의 agents.* 동시성/깊이/역할 설정, Chat Completions provider 지원 deprecation, Sites hosting.json/secret 분리를 반영했습니다.",
        changelog_item_6: "Chrome 소셜 확인에서 X의 usage reset류 claim은 검증하지 않았고, LinkedIn은 공식 Sites/plugins 반복과 파트너 사례만 확인했으며 Threads는 로드 제한으로 제외했습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome에서 X와 LinkedIn을 읽었고 Threads는 로드 제한으로 본문 확인에 실패했습니다. X에는 usage reset류 비공식 claim이 있었고, LinkedIn은 공식 Sites/plugins 발표 반복과 파트너 사례를 보여줬습니다. 검증되지 않은 소셜 claim은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The June 4 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available. The models page now also makes the arbitrary-provider Chat Completions path a deprecated Codex support path that will be removed in a future release.",
        ...
        changelog_item_2: "Removed stale Pro promo wording and aligned the current Pro 5x/20x structure, Go/iOS wording, ChatGPT for Excel shared limits, and credit scope.",
        ...
        changelog_item_4: "Added the May 13 security note that macOS users should update Codex App/CLI through official paths by June 12, 2026 for certificate rotation.",
        changelog_item_5: "Reflected config-reference agents.* concurrency/depth/role settings, Chat Completions provider deprecation, and Sites hosting.json versus secret handling.",
        changelog_item_6: "Chrome social verification saw unverified X usage-reset claims, LinkedIn official/partner repeats, and a Threads load timeout; no social-only claim was added.",
        ...
        s18_field_credit: "Chrome social verification could read X and LinkedIn, while Threads timed out before a readable body loaded. X included unverified usage-reset claims, and LinkedIn repeated official Sites/plugins messaging plus partner examples. No social-only claim was added.",
