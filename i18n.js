...
const translations = {
    ko: {
        ...
        overview_update_4_desc: "May 21 Enterprise/Edu 릴리스 노트와 May 22 Gartner 발표를 확인해 Goal mode GA, Appshots, locked computer use, in-app browser annotations, admin Codex analytics, plugin sharing 기본값, enterprise governance, sandboxing, RBAC, auditability를 일반 사용자 기능과 분리했습니다.",
        overview_update_5_title: "관련 발표 전체 재점검",
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 5월 22일 Gartner enterprise coding agents 발표는 Codex가 enterprise governance, sandboxing, flexible deployment, App/IDE/CLI/SDK/cloud surface, approval gates, RBAC, auditable governance로 평가받았다는 맥락으로 추가했습니다. 2026년 5월 18일 Dell Technologies 파트너십 발표는 Codex를 하이브리드·온프레미스 기업 환경과 Dell AI Data Platform/AI Factory에 더 가깝게 연결하려는 enterprise 배포 방향으로 정리했고, 일반 사용자에게 즉시 켜지는 기능처럼 쓰지는 않았습니다.",
        overview_update_6_title: "Docs MCP·API changelog·설정 기준 재점검",
        overview_update_6_desc: "Docs MCP와 config reference를 다시 확인해 project-scoped `.codex/config.toml`이 trusted project에서만 로드되고 machine-local provider/auth/profile/telemetry 키는 user-level config에 둬야 한다는 제한, shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `approval_policy.granular`, `approvals_reviewer`, `default_permissions`, permissions profile, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, hooks/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. OpenAI API changelog와 deprecations 문서도 별도로 확인해 `gpt-5-codex`, `gpt-5.1-codex*`, `gpt-5.2-codex` API snapshot의 2026년 7월 23일 종료 예정과 `gpt-5.5` 대체 안내를 Platform/API 개발자 참고사항으로 분리했습니다. Secure MCP Tunnel은 enterprise 고객용 private MCP 연결 옵션으로, Codex 일반 설정이 아니라 Platform/API lane에만 둡니다.",
        ...
        changelog_item_3: "May 21 Enterprise/Edu 릴리스 노트를 반영해 Goal mode GA, Appshots, locked computer use, in-app browser annotations, admin analytics, plugin sharing 기본값을 추가했습니다.",
        changelog_item_4: "May 22 Gartner enterprise coding agents 발표를 enterprise governance, sandboxing, flexible deployment, RBAC, auditability 맥락으로 분리했습니다.",
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog/deprecations, API 모델 문서, Windows sandbox 발표, Dell/Gartner enterprise 발표를 다시 확인하고 Secure MCP Tunnel은 Platform/API enterprise 항목으로만 분리했습니다.",
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn을 확인했습니다. X는 읽혔지만 'Goals' 일반 결과가 섞였고, Threads는 일반 커뮤니티/템플릿 글, LinkedIn은 일치 결과 없음이었습니다. 검증되지 않은 소셜 claim은 추가하지 않았습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome 연결이 성공했고 X, Threads, LinkedIn 검색 결과를 확인했습니다. X는 일반 'Goals' 결과가 많이 섞였고, Threads는 일반 커뮤니티/템플릿 글, LinkedIn은 일치 결과 없음이었습니다. 검증되지 않은 주장은 추가하지 않았고, 공식 문서와 맞는 권한 확인, 출력 계약, 도구 사용 규칙, 완료 기준, OpenAI Developers plugin 활용 메모만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 27 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. The May 27 check records that Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 includes 20x Plus on an ongoing basis and keeps higher 5-hour Codex limits at 25x Plus through May 31, 2026. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        ...
        overview_update_4_desc: "The May 21 Enterprise/Edu release notes and May 22 Gartner announcement are treated as enterprise context: Goal mode GA, Appshots, locked computer use, in-app browser annotations, admin Codex analytics, plugin sharing defaults, enterprise governance, sandboxing, RBAC, and auditability.",
        overview_update_5_title: "Broader launch-post sweep",
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The May 22 Gartner enterprise coding agents announcement is treated as enterprise context for governance, sandboxing, flexible deployment, approval gates, RBAC, auditability, and the Codex App/IDE/CLI/SDK/cloud surface. The May 18 Dell Technologies partnership post stays an enterprise deployment direction for hybrid and on-prem environments with Dell AI Data Platform / AI Factory, not as a general user feature to enable today.",
        overview_update_6_title: "Docs MCP, API changelog, and config rechecked",
        overview_update_6_desc: "Docs MCP and the config reference were rechecked around trusted project-scoped `.codex/config.toml`, the user-level requirement for machine-local provider/auth/profile/telemetry routing keys, shared CLI/IDE setup, the AGENTS.md steering snippet, `review_model`, granular approvals, `approvals_reviewer`, `default_permissions`, permissions profiles, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, and hooks/plugin/skill controls. The OpenAI API changelog and deprecations docs were also checked: `gpt-5-codex`, `gpt-5.1-codex*`, and `gpt-5.2-codex` API snapshots are listed for July 23, 2026 shutdown with `gpt-5.5` as the substitute, which stays in the Platform/API lane rather than the Codex model-picker lane. Secure MCP Tunnel is also documented only as an enterprise Platform/API option for private MCP connectivity through an outbound customer-hosted `tunnel-client`.",
        ...
        changelog_item_3: "Added May 21 Enterprise/Edu notes for Goal mode GA, Appshots, locked computer use, in-app browser annotations, admin analytics, and plugin sharing defaults.",
        changelog_item_4: "Separated the May 22 Gartner enterprise coding agents announcement as enterprise governance, sandboxing, flexible deployment, RBAC, and auditability context.",
        changelog_item_5: "Rechecked Docs MCP, best practices, config, the OpenAI API changelog/deprecations, API model docs, the Windows sandbox post, and Dell/Gartner enterprise announcements; Secure MCP Tunnel stays in the Platform/API enterprise lane.",
        changelog_item_6: "The Chrome plugin checked X, Threads, and LinkedIn. X was readable but polluted by generic 'Goals' results, Threads showed general community/template posts, and LinkedIn returned no matching result. No unverified social claim was added.",
        ...
        s18_field_credit: "Chrome connected successfully in this run, and X, Threads, and LinkedIn search results were checked. X was polluted by generic 'Goals' results, Threads showed general community/template posts, and LinkedIn returned no matching content. I did not add unverified claims; I only tied official-doc-aligned notes on permissions, output contracts, tool-use rules, done criteria, and the OpenAI Developers plugin back to existing guidance.",
