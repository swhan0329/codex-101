...
const translations = {
    ko: {
        ...
        changelog_item_2: "`codex/pricing`의 Plus/Pro/Business GPT-5.5 사용량 표와 API Key Not available 표기를 분리해서 설명했습니다.",
        changelog_item_3: "Quickstart의 데스크톱 App 우선 설치 흐름과 App/IDE/CLI/Cloud 선택 구조를 다시 맞췄습니다.",
        changelog_item_4: "Config reference 기준으로 profile 설정을 별도 `~/.codex/<profile>.config.toml` 파일 방식으로 고치고, stale `features.prevent_idle_sleep`/`features.network_proxy` 예시를 제거했습니다.",
        ...
        changelog_item_6: "Chrome으로 X, Threads, LinkedIn을 다시 확인했고, 새 social-only claim은 추가하지 않은 채 공식 문서로 확인되는 실무 습관만 유지했습니다. OpenAI의 6월 9일 customer stories는 workflow 예시로만 추가했습니다.",
        ...
        field_tips_intro: "2026-06-10 Chrome 로그인 세션으로 X, Threads, LinkedIn을 확인했지만 새로 넣을 만한 공식 근거의 social-only claim은 없었습니다. 아래에는 이미 공식 문서와 대조해 둔 사용 습관만 남겼습니다.",
    ...
    en: {
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The June 2 `Codex for every role, tool, and workflow` launch remains official announcement language for role-specific plugins, Sites preview, and broader annotations. The same-day `Codex is becoming a productivity tool for everyone` post remains report context for knowledge-worker use across data analysis, research, knowledge artifacts, workflow automation, and lightweight tools. The June 9 Nextdoor, Notion, and Wasmer customer stories are reflected only as workflow examples for customer-feedback analysis, workflow-automation development, and WebAssembly app development, not as new platform, plan, or feature-flag changes. The May 13 TanStack / Mini Shai-Hulud response stays as a security operations note: macOS users should update OpenAI macOS apps, including Codex App and Codex CLI, from official paths by June 12, 2026.",
        ...
        changelog_item_2: "Separated `codex/pricing` Plus/Pro/Business GPT-5.5 usage ranges from the API Key table where GPT-5.5 still shows as not available.",
        ...
        changelog_item_4: "Aligned config examples with the current profile-file pattern, removed stale `features.prevent_idle_sleep` / `features.network_proxy` examples, and kept `experimental_instructions_file` deprecation guidance.",
        ...
        changelog_item_6: "Rechecked X, Threads, and LinkedIn in Chrome, added no new social-only claims, and kept only official-doc-corroborated practice patterns. OpenAI's June 9 customer stories were added only as workflow examples.",
        ...
        field_tips_intro: "On June 10, 2026, I checked recent X, Threads, and LinkedIn Codex posts through Chrome, then kept only patterns that were corroborated by official docs.",
