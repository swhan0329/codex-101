...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 7월 10일 기준으로 가장 먼저 다시 확인했고, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`가 새 추천 모델 family로 올라온 것을 반영했습니다. 기본 Power setting은 `gpt-5.6-sol` medium reasoning이고, Sol은 복잡한 coding/computer use/research/security, Terra는 everyday workhorse, Luna는 빠르고 저렴한 high-volume 작업에 맞습니다. `gpt-5.5`는 previous-generation frontier model로 남고, `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 인증 Codex 기준 deprecated로 유지됩니다.",
        ...
        overview_fact_2_desc: "오늘 기준 `codex/models`, `What's new`, API latest-model 문서는 모두 GPT-5.6을 현재 모델 축으로 다룹니다. Codex 제품에서는 Sol/Terra/Luna를 추천 모델로 고르고, API에서는 `gpt-5.6` alias가 `gpt-5.6-sol`로 라우팅됩니다. 직접 API 앱에는 Programmatic Tool Calling, Multi-agent beta, explicit prompt caching, persisted reasoning, max reasoning effort, Pro mode 같은 API-only 기능도 있으므로 Codex 제품 사용법과 섞지 않습니다.",
        overview_fact_3_title: "플랜/팀 요금제 보강",
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고, 데스크톱에서는 App 설치 경로를 먼저 보여주되 IDE extension, CLI, Cloud도 정식 시작 경로로 유지합니다. `codex/pricing`은 Plus / Pro 5x / Pro 20x / Business / Enterprise & Edu / API Key 표를 유지하며 GPT-5.6 Sol/Terra/Luna의 local-message/API Key usage-based 범위를 표시합니다. Pro에는 `gpt-5.3-codex-spark` research preview가 별도 혜택으로 남아 있습니다. Fast mode는 아직 GPT-5.5/GPT-5.4만 지원하므로 추천 모델, 사용량 표, speed mode, API 가격표를 분리해서 확인해야 합니다.",
        ...
        changelog_item_1: "`codex/models`를 먼저 재확인했고, `gpt-5.6-sol`/`gpt-5.6-terra`/`gpt-5.6-luna` 추천 전환과 `gpt-5.5`의 previous-generation 위치를 반영했습니다.",
        ...
        changelog_item_3: "Quickstart, App/IDE/CLI/Cloud, Remote connections, Windows, Sites, config, MCP/hooks/plugins/skills 문서를 다시 훑었고, config/remote-control/Sites/Windows 활성화 절차와 충돌하는 새 공식 변경은 없었습니다. 7월 9일 Codex changelog의 ChatGPT desktop app 통합, inline annotation/editing, sidebar PR review, multi-repo project, GPT-5.6 Computer Use 개선을 반영했습니다.",
        changelog_item_4: "공식 use-case catalog 60개를 다시 감사했고, 6개 Starter prompt drift를 반영한 뒤 local/official ID 차이 0, `promptEn`/`promptKo` 누락 0, official prompt mismatch 0, multiline 한국어 구조 회귀 0을 확인했습니다.",
        changelog_item_5: "OpenAI Developers/blog와 openai.com/index를 확인했습니다. 7월 8일 GPT-5.6 출시, ChatGPT work 확장, API latest-model/changelog를 반영했습니다. openai.com/index 직접 fetch 403은 기록하고 공식 검색/MCP 근거로 보완했습니다.",
        changelog_item_6: "Chrome으로 사람별 X/LinkedIn profile sweep을 다시 했습니다. 새로 공개 가이드에 승격할 검증된 person-level Codex 팁은 없어서 practitioner cards는 유지하고 로컬 research note만 업데이트했습니다.",
        ...
        s7_intro: 'Codex App은 여러 프로젝트를 넘나들며 병렬 agent thread를 운영하고, built-in Git와 integrated terminal로 결과를 리뷰하며, chats·artifact preview·in-app browser·browser use·computer use·image generation·memories·thread automations를 한곳에서 다루는 데 가장 적합한 데스크톱 표면입니다. 최신 App 문서와 2026년 6월 11일 26.609 릴리스 기준으로 Developer mode, App composer `/init`, plugin management 개선, unread chats, Profile token/activity 통계, 더 빠른 Browser use까지 함께 보는 것이 현재 상태에 가깝습니다. 로컬 작업은 기본적으로 cached web search를 쓰며, Windows에서는 네이티브 sandbox 또는 WSL2 경로와 Computer Use per-app access control을 함께 검토해야 합니다.',
        ...
        s17_intro_desc: "GPT-5.6 시대의 핵심은 세세한 절차를 모두 적는 것보다 목표 결과, 성공 기준, 제약, 사용할 수 있는 근거를 분명히 주고 모델이 적절한 경로를 고르게 하는 데 있습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The July 10 run started by re-checking `codex/models` first and reflected the new GPT-5.6 recommended family: `gpt-5.6-sol`, `gpt-5.6-terra`, and `gpt-5.6-luna`. Default Power uses Sol with medium reasoning; GPT-5.5 is now previous-generation, while `gpt-5.2` and `gpt-5.3-codex` remain deprecated for ChatGPT-authenticated Codex.",
        ...
        changelog_item_1: "Rechecked `codex/models` first and reflected the GPT-5.6 Sol/Terra/Luna recommendation shift plus GPT-5.5 as previous-generation.",
        ...
        changelog_item_3: "Rechecked Quickstart, App/IDE/CLI/Cloud, Remote connections, Windows, Sites, config, MCP, hooks, plugins, and skills; no new official change conflicted with config, remote-control, Sites, or Windows setup wording. Codex CLI 0.142.5 is noted separately as a trace-log payload fix.",
        changelog_item_4: "Audited all 60 official use cases again and confirmed 0 local/official ID differences, 0 `promptEn`/`promptKo` gaps, 0 official prompt mismatches, and 0 multiline Korean prompt regressions.",
        changelog_item_5: "Rechecked OpenAI Developers/blog and openai.com/index. GPT-Live is a ChatGPT Voice launch with API/Codex support still described as in progress, while the coding-evaluation post and AP+ customer story are evaluation/enterprise context rather than Codex setup changes.",
        changelog_item_6: "Rechecked the person-level practitioner baseline. No newly verified Codex tip needed promotion into the public guide, so the practitioner cards stay stable and the local research note was updated.",
        ...
        s3_rec_note: "💡 The latest `codex/models` guidance says to start with the GPT-5.6 family. Default Power uses `gpt-5.6-sol` with medium reasoning; choose Sol for difficult or high-value work, Terra for everyday work, and Luna for fast, repeatable, high-volume tasks. Pricing now lists GPT-5.6 Sol/Terra/Luna in Plus, Pro, Business, and API Key usage tables. Fast mode is still documented for GPT-5.5/GPT-5.4 only, so keep model choice and speed mode as separate decisions. `gpt-5.5` remains available as the previous-generation frontier model, `gpt-5.3-codex-spark` remains a Pro research preview, and `gpt-5.2`/`gpt-5.3-codex` remain deprecated for ChatGPT-authenticated Codex.",
        computer_use_kicker: "Computer use",
        ...
        s7_title: "Codex App (macOS + Windows)",
        s7_intro: 'The Codex App is the best desktop surface for working across projects, running parallel agent threads, reviewing results with built-in Git and the integrated terminal, and using chats, artifact preview, the in-app browser, browser use, computer use, image generation, memories, and thread automations in one place. The latest app docs and the June 11, 2026 26.609 release add Developer mode, app-composer `/init`, plugin-management improvements, unread chats, Profile token/activity stats, and faster Browser use to the current workflow story. Local work uses cached web search by default; on Windows, review the native sandbox or WSL2 path plus Computer Use per-app access controls.',
        ...
        s17_intro_desc: "For the GPT-5.6 era, the highest-leverage prompts define the target outcome, success criteria, constraints, and available evidence, then let the model choose the right path.",
