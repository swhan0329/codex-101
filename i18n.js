...
const translations = {
    ko: {
        ...
        overview_update_2_desc: "개인 플랜 한도뿐 아니라 `GPT-5.5` 사용량 표, `Free $0`·`Go $8`·`Plus $20`·`Pro from $100`, Business·Enterprise의 Codex-only pay-as-you-go seat, 그리고 현재 `codex/pricing` 문서의 Plus / Pro 5x / Pro 20x / Business / API Key 구성을 다시 맞췄습니다. 특히 Codex API-key 로그인에서는 `gpt-5.5`가 제공되지 않는 반면, 직접 OpenAI API의 Models 문서에서는 `gpt-5.5`와 `gpt-5.5-pro`를 별도로 볼 수 있다는 차이를 가격 섹션에서 더 또렷하게 분리했습니다.",
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 5월 4일 저지연 voice engineering 글은 Codex 문서에 직접 반영할 내용은 아니었고, Codex 관련 핵심 발표는 4월 30일 `Advanced Account Security`, 4월 28일 AWS, 4월 23일 GPT-5.5, 4월 16일 `Codex for (almost) everything`, 4월 2일 pay-as-you-go 묶음으로 유지되는 것을 확인했습니다.",
        ...
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. `codex/pricing`은 오늘도 Plus / Pro 5x / Pro 20x / Business / API Key 표와 Plus 로컬 메시지 범위(`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150)를 유지합니다. 다만 Codex API-key 로그인은 `gpt-5.5`가 제공되지 않고, 직접 OpenAI API의 `gpt-5.5`/`gpt-5.5-pro` 모델 접근은 별도 API 문서 기준으로 봐야 합니다. 따라서 실무 문서는 사용량 표, token credit 표, usage dashboard, API 모델 표를 섞지 말고 함께 확인하도록 고쳤습니다.",
        ...
        changelog_item_1: "`gpt-5.5` 우선 추천을 다시 확인하고, Codex 모델 선택에서는 ChatGPT 로그인 경로만 해당된다는 점을 직접 OpenAI API 모델 접근과 분리했습니다.",
        changelog_item_2: "가격 설명에서 Codex API-key 로그인은 `gpt-5.5`를 제공하지 않는다는 점과 직접 API 모델 표는 별도라는 점을 더 선명하게 고쳤습니다.",
        ...
        changelog_item_5: "Docs MCP, best practices, config, plugins, skills, approvals 문서를 다시 확인하고 `requirements.toml` 기반 관리자 제어 포인트를 추가했습니다.",
        ...
        s4_note_access: "현재 공식 문서는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. 처음 보는 사용자라면 `Free $0`, `Go $8`, `Plus $20`, `Pro from $100` 순으로 보면 이해가 쉽고, 팀 사용자는 Business·Enterprise의 Codex-only pay-as-you-go seat를 같이 보면 됩니다. Pricing 페이지는 Plus / Pro 5x / Pro 20x / Business / API Key 표와 모델별 5시간 한도를 보여주며, Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. 중요한 차이는 Codex의 API-key 로그인 표에서는 `gpt-5.5`가 제공되지 않는다는 점입니다. 반면 직접 OpenAI API의 별도 Models/Pricing 문서에서는 `gpt-5.5`와 `gpt-5.5-pro`를 확인해야 합니다. 실제 비용 판단은 메시지/작업 한도, token credit rate card, usage dashboard, API 모델 가격표를 섞지 말고 함께 대조하세요. 이미지 생성은 일반 Codex 한도를 평균 3-5배 더 빨리 쓰며 Fast mode는 지원 모델에서 더 높은 비율로 크레딧을 소모합니다.",
        ...
        s4_codex_api: "CLI/IDE/SDK 전용, 클라우드 기능 없음. Codex API-key 로그인은 `gpt-5.5` 제외, 직접 OpenAI API는 별도 모델 표 확인",
        // Section 4
        s5_title: "설치 및 인증",
        ...
        s13_c4_d: "Business/Enterprise 운영자는 admin-enforced requirements로 approval, sandbox, web_search, MCP allowlist, Browser Use, Computer Use, in-app browser 같은 기능을 사용자 설정보다 우선 적용할 수 있습니다.",
    ...
    en: {
        ...
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, the Business/Enterprise Codex-only pay-as-you-go path, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. This run also makes the split clearer: Codex API-key sign-in does not expose `gpt-5.5`, while direct OpenAI API model access is documented separately.",
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The May 4 low-latency voice engineering post does not need a Codex 101 change; the Codex-relevant launch sweep still centers on the April 30 Advanced Account Security launch, April 28 AWS launch, April 23 GPT-5.5 launch, April 16 `Codex for (almost) everything`, and April 2 pay-as-you-go pricing launch.",
        ...
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / API Key tables and the published Plus local-message ranges (`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150). The important split is that Codex API-key sign-in does not expose `gpt-5.5`; direct OpenAI API access to `gpt-5.5` and `gpt-5.5-pro` belongs to the separate API Models/Pricing docs. The guide now tells readers to check usage tables, token credit rates, the usage dashboard, and API model pricing without collapsing them into one bucket.",
        ...
        changelog_item_1: "Reconfirmed `gpt-5.5` as the top recommendation and separated Codex model-picker availability from direct OpenAI API model access.",
        changelog_item_2: "Clarified that Codex API-key sign-in does not expose `gpt-5.5`, while the direct API model table should be read separately.",
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, plugins, skills, and approvals docs, then added the `requirements.toml` admin-control note.",
        ...
        s4_codex_api: "CLI/IDE/SDK only, no cloud features. Codex API-key sign-in excludes GPT-5.5; check direct OpenAI API models separately",
        ...
        s13_c4_d: "Business and Enterprise admins can use admin-enforced requirements to pin approval, sandbox, web_search, MCP allowlists, Browser Use, Computer Use, and in-app browser availability above user config.",
