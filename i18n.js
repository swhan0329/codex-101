...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 6월 23일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 현재 모델 선택지와 API availability는 범위가 다를 수 있으므로, Codex product model picker, ChatGPT 일반 모델 retirement, API 모델 제공 여부를 분리해서 읽어야 합니다.",
        ...
        changelog_item_1: "`codex/models`를 먼저 재확인했고, `gpt-5.5` 우선 추천과 `gpt-5.2`의 ChatGPT 인증 Codex deprecated 상태가 유지되는지 확인했습니다.",
        ...
        s3_rec_note: "💡 최신 `codex/models` 기준 공식 가이드는 모델 선택기에서 `gpt-5.5`가 보이면 대부분의 작업을 거기서 시작하라고 권장합니다. 오늘은 같은 모델 페이지가 API Access를 true로 표시하고 pricing의 Plus/Pro/Business 사용량 표도 GPT-5.5 local-message 범위를 보여주며, API Key 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. API-key 사용자는 실제 Codex 모델 선택기와 표준 API pricing을 같이 확인하세요. 비용/속도 균형이 중요하면 `gpt-5.4`를 계속 사용하면 됩니다. API에서는 `gpt-5.5`와 `gpt-5.5-pro`를 사용할 수 있으며, Pro는 더 높은 정확도가 필요한 Responses API/Batch API 작업에 적합합니다. 더 빠르고 가벼운 코딩 작업이나 subagent에는 `gpt-5.4-mini`, 거의 즉각적인 반복에는 ChatGPT Pro research preview인 `gpt-5.3-codex-spark`를 검토하세요. `gpt-5.2`는 ChatGPT 로그인 Codex 기준 deprecated이고, ChatGPT 릴리스 노트는 GPT-5.2 Instant/Thinking/Pro가 2026년 6월 12일부터 ChatGPT에서 retired됐다고 별도로 설명합니다.",
    ...
    en: {
        ...
        changelog_item_1: "Rechecked `codex/models` first and confirmed `gpt-5.5` is still the top recommendation while `gpt-5.2` remains deprecated for ChatGPT-authenticated Codex.",
