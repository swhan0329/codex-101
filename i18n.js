...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 7월 9일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 인증 Codex 기준 deprecated로 분리했고, API code-generation 문서의 일부 legacy codex-family 언급은 Codex product model picker 안내와 섞지 않도록 별도 Platform/API 범위로 남겼습니다.",
        overview_update_2_title: "요금제·크레딧 구조 재정리",
        overview_update_2_desc: "개인 플랜 한도뿐 아니라 GPT-5.5 pricing scope를 다시 나눴습니다. 현재 pricing은 Free $0, Go $8, Plus $20, Pro from $100, Business, Enterprise & Edu, API Key 구조를 보여줍니다. Plus/Pro/Business 사용량 표에는 GPT-5.5 local-message 범위가 보이고, API Key 사용량 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. Help Center Codex rate card는 대부분 고객에게 token-based pricing을 기본으로 안내하며, GPT-5.5는 1M tokens 기준 input 125 credits, cached input 12.50 credits, output 750 credits로 표시됩니다. Fast mode는 GPT-5.5/GPT-5.4를 1.5배 빠르게 하지만 각각 standard 대비 2.5배/2배 credit을 소모하고, API-key Codex에는 적용되지 않습니다. legacy average-message rate는 migration 전인 일부 Enterprise 고객 범위로만 남겼습니다.",
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, Codex changelog, `developers.openai.com/blog`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 7월 8일 OpenAI 발표 중 GPT-Live는 현재 ChatGPT Voice 출시이고 API/Codex 지원은 작업 중이라고 분리했습니다. `Separating signal from noise in coding evaluations`는 Codex 기반 investigator agent가 평가 문제를 감사한 방법론 글로, `Australian Payments Plus`는 ChatGPT Enterprise와 Codex 고객 사례로만 반영했습니다. 둘 다 Codex 모델 순서, 활성화 절차, credential/permission 변경은 아닙니다.",
        ...
        overview_fact_1_desc: "`codex/models`를 오늘 다시 확인한 결과, 최우선 추천 모델은 여전히 `gpt-5.5`입니다. Codex 모델 선택기에 보이면 대부분의 작업을 거기서 시작하고, 아직 계정에 보이지 않으면 `gpt-5.4`를 대체 선택지로 사용합니다. 오늘 주의할 차이는 `codex/models`가 `gpt-5.5`의 API Access를 true로 표시하고 ChatGPT 또는 API-key authentication 경로를 언급하며, `codex/pricing`도 Plus/Pro/Business 표에는 GPT-5.5 local-message 범위를, API Key 표에는 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시한다는 점입니다. 그래서 API-key 사용자는 실제 모델 선택기와 표준 API 가격표를 함께 확인해야 합니다. `gpt-5.4-mini`는 빠르고 효율적인 로컬/subagent 선택지이고, `gpt-5.3-codex-spark`는 ChatGPT Pro research preview입니다. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 인증 Codex에서는 deprecated로 표시되고, ChatGPT 릴리스 노트는 GPT-5.2 Instant/Thinking/Pro가 2026년 6월 12일부터 ChatGPT에서 retired됐다고 별도로 설명합니다.",
        overview_fact_2_title: "GPT-5.5 승격과 대체 선택지 구조",
        overview_fact_2_desc: "오늘 기준 `Introducing GPT-5.5`, 최신 `codex/models`, API Models 문서를 함께 보면 `gpt-5.5`가 복잡한 코딩·computer use·knowledge work·research workflow의 최우선 추천 모델이라는 점이 유지됩니다. 다만 오늘 문서 정리에서는 “Codex 안에서 고르는 모델”, “API-key authentication으로 Codex를 쓰는 경우”, “직접 API에서 호출 가능한 모델”을 명확히 분리했습니다. API 자체에서는 `gpt-5.5`와 `gpt-5.5-pro`를 별도 모델로 사용할 수 있고, GPT-5.6 Sol은 select trusted partners/organizations preview와 coming-soon availability 범위로만 설명됩니다. Codex API-key 사용자는 모델 페이지와 가격표의 차이를 함께 확인해야 합니다. `gpt-5.4`는 rollout 계정이나 비용/속도 기준에서 여전히 강력한 대체 선택지이고, `gpt-5.4-mini`는 빠른 작업과 subagent용 효율 모델로 남아 있습니다.",
        overview_fact_3_title: "플랜/팀 요금제 보강",
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고, 데스크톱에서는 App 설치 경로를 먼저 보여주되 IDE extension, CLI, Cloud도 정식 시작 경로로 유지합니다. `codex/pricing`은 Plus / Pro 5x / Pro 20x / Business / Enterprise & Edu / API Key 표를 유지하고, Plus 설명에는 GPT-5.5, GPT-5.4, GPT-5.4 mini가 표시됩니다. 사용량 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini 중심이고, Pro에는 `gpt-5.3-codex-spark` research preview가 별도 혜택으로 남아 있습니다. Help Center Codex rate card는 token-based pricing을 대부분 고객의 기본 rate card로 설명하고 GPT-5.5를 1M tokens 기준 input 125 credits, cached input 12.50 credits, output 750 credits로 표시합니다. legacy average-message rate는 migration 전인 일부 Enterprise 고객에게만 남아 있으므로, 사용량 표, token credit 표, usage dashboard, App `Settings > Profile`의 lifetime tokens/token activity, 실제 모델 선택기, API 모델 가격표를 섞지 말고 함께 확인해야 합니다.",
        ...
        changelog_item_1: "`codex/models`를 먼저 재확인했고, `gpt-5.5` 우선 추천과 `gpt-5.2`/`gpt-5.3-codex`의 ChatGPT 인증 Codex deprecated 상태를 반영했습니다.",
        changelog_item_2: "Pricing/rate card와 Speed 문서를 다시 확인했고 Free/Go 플랜, GPT-5.5 usage-based/API Key 범위, 125/12.50/750 credits per 1M tokens, Fast mode 2.5x/2x credit 소모를 분리했습니다.",
        changelog_item_3: "Quickstart, App/IDE/CLI/Cloud, Remote connections, Windows, Sites, config, MCP/hooks/plugins/skills 문서를 다시 훑었고, config/remote-control/Sites/Windows 활성화 절차와 충돌하는 새 공식 변경은 없었습니다. Codex CLI 0.142.5의 trace log payload fix는 보안/로그 hygiene 메모로 분리했습니다.",
        changelog_item_4: "공식 use-case catalog 60개를 다시 감사했고, local/official ID 차이 0, `promptEn`/`promptKo` 누락 0, official prompt mismatch 0, multiline 한국어 구조 회귀 0을 확인했습니다.",
        changelog_item_5: "OpenAI Developers/blog와 openai.com/index를 확인했습니다. 7월 8일 GPT-Live는 ChatGPT Voice 중심 출시이고 API/Codex 지원은 작업 중으로 분리했으며, coding evaluation 글과 AP+ 고객 사례는 제품 설정 변경이 아닌 평가/엔터프라이즈 맥락으로만 반영했습니다.",
        changelog_item_6: "사람별 practitioner baseline을 다시 대조했습니다. 새로 공개 가이드에 승격할 검증된 person-level Codex 팁은 없어서, 사람별 practitioner cards는 유지하고 로컬 research note만 업데이트했습니다.",
        ...
        s3_rec_note: "💡 최신 `codex/models` 기준 공식 가이드는 모델 선택기에서 `gpt-5.5`가 보이면 대부분의 작업을 거기서 시작하라고 권장합니다. 오늘은 같은 모델 페이지가 API Access를 true로 표시하고 pricing의 Plus/Pro/Business 사용량 표도 GPT-5.5 local-message 범위를 보여주며, API Key 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. API-key 사용자는 실제 Codex 모델 선택기와 표준 API pricing을 같이 확인하세요. 비용/속도 균형이 중요하면 `gpt-5.4`를 계속 사용하면 됩니다. API에서는 `gpt-5.5`와 `gpt-5.5-pro`를 사용할 수 있으며, GPT-5.6 Sol은 아직 제한 preview/coming-soon 범위로 분리해야 합니다. 더 빠르고 가벼운 코딩 작업이나 subagent에는 `gpt-5.4-mini`, 거의 즉각적인 반복에는 ChatGPT Pro research preview인 `gpt-5.3-codex-spark`를 검토하세요. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 로그인 Codex 기준 deprecated이고, ChatGPT 릴리스 노트는 GPT-5.2 Instant/Thinking/Pro가 2026년 6월 12일부터 ChatGPT에서 retired됐다고 별도로 설명합니다.",
        ...
        s4_note_access: "현재 공식 문서는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. 처음 보는 사용자라면 `Free $0`, `Go $8`, `Plus $20`, `Pro from $100` 순으로 보면 이해가 쉽고, 팀 사용자는 Business·Enterprise의 Codex-only pay-as-you-go seat를 같이 보면 됩니다. Pricing 페이지는 Plus / Pro 5x / Pro 20x / Business / API Key 표와 모델별 5시간 한도를 보여줍니다. Help Center Codex rate card는 대부분 고객에게 token-based pricing을 기본으로 안내하고, GPT-5.5를 1M tokens 기준 input 125 credits, cached input 12.50 credits, output 750 credits로 표시합니다. 실제 비용 판단은 메시지/작업 한도, token credit rate card, usage dashboard, 실제 모델 선택기, API 모델 가격표를 섞지 말고 함께 대조하세요. 2026년 6월 11-24일 eligible Plus/Pro 초대 reset banking은 dialog와 current terms로 조건을 확인해야 합니다. 이미지 생성은 일반 Codex 한도를 평균 3-5배 더 빨리 쓰며 Fast mode는 지원 모델에서 더 높은 비율로 크레딧을 소모합니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The July 9 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and that `gpt-5.4-mini` and `gpt-5.3-codex-spark` keep their fast local/subagent and Pro research-preview roles. `gpt-5.2` and `gpt-5.3-codex` remain deprecated for ChatGPT-authenticated Codex. Some API code-generation guidance still contains legacy codex-family language, so the guide keeps Codex product model-picker guidance and Platform/API guidance in separate lanes.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps GPT-5.5 pricing scope split cleanly. The current pricing page shows Free $0, Go $8, Plus $20, Pro from $100, Business, Enterprise & Edu, and API Key. Plus/Pro/Business usage-limit tables show GPT-5.5 local-message ranges, and the API Key usage table lists GPT-5.5/GPT-5.4/GPT-5.4 mini as usage-based. The Help Center Codex rate card matters for rollout planning: most customers should use the token-based pricing rate card, GPT-5.5 is listed at 125 input / 12.50 cached input / 750 output credits per 1M tokens, and Fast mode speeds GPT-5.5/GPT-5.4 by 1.5x while consuming 2.5x/2x standard credits. Fast mode does not apply to API-key Codex, which uses standard API pricing.",
        ...
        overview_fact_1_desc: "Re-checking `codex/models` confirms that `gpt-5.5` is still the top recommendation. Start there when it appears in the Codex model picker, and use `gpt-5.4` when it has not rolled out to the account yet. One clarification now matters more in the guide: the Codex models page marks `gpt-5.5` API Access as true and mentions ChatGPT or API-key authentication, and `codex/pricing` shows GPT-5.5 local-message ranges for Plus, Pro, and Business while the API Key usage table lists GPT-5.5/GPT-5.4/GPT-5.4 mini as usage-based. API-key users should verify the actual picker and standard API pricing before relying on it. `gpt-5.4-mini` stays the fast local/subagent option, `gpt-5.3-codex-spark` remains the ChatGPT Pro research preview, `gpt-5.2` and `gpt-5.3-codex` are deprecated for ChatGPT-authenticated Codex, and GPT-5.2 Instant/Thinking/Pro are separately retired in ChatGPT as of June 12, 2026.",
        overview_fact_2_title: "GPT-5.5 promoted, GPT-5.4 remains important",
        overview_fact_2_desc: "The latest model story is now split more clearly. `gpt-5.5` is still the top recommendation for complex coding, computer use, knowledge work, and research workflows in Codex. In this refresh, the guide separates the Codex model picker, API-key authentication for Codex, and direct API model calls. The API itself exposes `gpt-5.5` and `gpt-5.5-pro` as separate models, while GPT-5.6 Sol is currently a limited select-partner/organization preview with broader ChatGPT, Codex, and API availability described as coming soon. API-key Codex users should compare the models page, pricing table, and actual product picker. `gpt-5.4` still matters for rollout, cost, or latency tradeoffs, while `gpt-5.4-mini` remains the efficient choice for lighter work and subagents.",
        overview_fact_3_title: "Plans and team pricing clarified",
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex and now puts the App path first on desktop platforms while keeping IDE extension, CLI, and Cloud as official choices. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / Enterprise & Edu / API Key tables. Plus lists latest models such as GPT-5.5, GPT-5.4, and GPT-5.4 mini, and usage tables center on GPT-5.5/GPT-5.4/GPT-5.4 mini. Pro still includes `gpt-5.3-codex-spark` as a research-preview benefit, so the guide separates pricing limits from current model recommendations. The Help Center rate card adds the operational pricing layer: GPT-5.5 is 125 input / 12.50 cached input / 750 output credits per 1M tokens, while the legacy average-message rate card is only for remaining unmigrated Enterprise customers.",
        ...
        overview_fact_6_desc: "The guide now keeps `review_model`, granular approvals, `approvals_reviewer`, top-level `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, `service_tier`, `personality`, `default_permissions`, Windows sandbox keys, `model_instructions_file`, `background_terminal_max_timeout`, `memories.disable_on_external_context`, `sqlite_home`, TUI settings, protected paths, JSON schema, app-connector controls, `features.fast_mode`, `features.code_mode`, `features.rollout_budget`, app-level approval modes, managed `allow_remote_control`, `features.browser_use_full_cdp_access`, permissions profiles, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace controls, OpenTelemetry export, plugin packaging, Bedrock provider setup, and the split cloud runtime aligned with current docs. Project-scoped config loads only for trusted projects, and provider/auth/profile/telemetry routing keys belong in user-level config. The Platform/API lane keeps GPT-5.5 latest-model guidance separate, including the medium reasoning default and image-detail behavior changes for direct API apps.",
        ...
        changelog_item_1: "Rechecked `codex/models` first and reflected `gpt-5.5` as the top recommendation while `gpt-5.2`/`gpt-5.3-codex` are deprecated for ChatGPT-authenticated Codex.",
        changelog_item_2: "Rechecked pricing, the rate card, and Speed; the guide now keeps Free/Go plans, GPT-5.5 usage-based/API Key scope, 125/12.50/750 credits per 1M tokens, and Fast mode 2.5x/2x credit use separate.",
        ...
        s3_rec_note: "💡 The latest `codex/models` guidance says to start with `gpt-5.5` when it appears in your picker. Today that page marks GPT-5.5 API Access as true, pricing shows GPT-5.5 local-message ranges for Plus/Pro/Business, and the API Key table lists GPT-5.5/GPT-5.4/GPT-5.4 mini as usage-based. API-key users should verify the actual Codex picker and standard API pricing. If cost/latency matters more, keep using `gpt-5.4`. In the API, `gpt-5.5` and `gpt-5.5-pro` are available, while GPT-5.6 Sol should still be treated as limited preview / coming-soon availability. Use `gpt-5.4-mini` for faster lighter tasks or subagents, and `gpt-5.3-codex-spark` for near-instant ChatGPT Pro research-preview iteration. `gpt-5.2` and `gpt-5.3-codex` are deprecated for Codex when signing in with ChatGPT, while GPT-5.2 Instant/Thinking/Pro are separately retired in ChatGPT as of June 12, 2026.",
        computer_use_kicker: "Computer use",
        ...
        s17_intro_desc: "For GPT-5.5, the highest-leverage prompts define the target outcome, success criteria, constraints, and available evidence, then let the model choose the right path.",
        ...
        s17_link2_kicker: "Official",
        s17_link2_title: "OpenAI Prompt Guidance",
        s17_link2_desc: "Read the primary guidance for GPT-5.5 outcome-first prompting, reasoning effort, phase, compaction, validation, and research/citation patterns.",
        ...
        pg_toc_lead: "This page reframes current Codex prompting around GPT-5.5 Prompt Guidance and the latest Codex best practices.",
        toc_prompt_1_t: "Behavior",
        ...
        pg_s1_intro: "With GPT-5.5, prompting is less about prescribing every step and more about defining the outcome, success criteria, and constraints clearly enough for the model to choose the right tool and reasoning path.",
        ...
        pg_s5_f3: "When migrating to GPT-5.5, switch the model first, keep reasoning fixed, run evals, and then tune one variable at a time.",
