...
const translations = {
    ko: {
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 5월 18일 Dell Technologies 파트너십 발표는 Codex를 하이브리드·온프레미스 기업 환경과 Dell AI Data Platform/AI Factory에 더 가깝게 연결하려는 enterprise 배포 방향으로 정리했고, 일반 사용자에게 즉시 켜지는 기능처럼 쓰지는 않았습니다. 2026년 5월 13일 `Building a safe, effective sandbox to enable Codex on Windows`는 dedicated sandbox user, firewall 기반 네트워크 차단, command runner를 포함한 `elevated` Windows sandbox 설계 근거로 유지했습니다.",
        overview_update_6_title: "Docs MCP·API changelog·설정 기준 재점검",
        overview_update_6_desc: "Docs MCP와 config reference를 다시 확인해 project-scoped `.codex/config.toml`이 trusted project에서만 로드되고 machine-local provider/auth/profile/telemetry 키는 user-level config에 둬야 한다는 제한, shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `approval_policy.granular`, `approvals_reviewer`, `default_permissions`, permissions profile, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, hooks/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. OpenAI API changelog와 deprecations 문서도 별도로 확인해 `gpt-5-codex`, `gpt-5.1-codex*`, `gpt-5.2-codex` API snapshot의 2026년 7월 23일 종료 예정과 `gpt-5.5` 대체 안내를 Platform/API 개발자 참고사항으로 분리했습니다.",
        ...
        changelog_item_3: "Remote connections 문서를 크게 재검토해 mobile host, 다른 Codex App 기기, SSH host, connected-host state, admin Remote Control/RBAC, Windows 운영 메모를 분리했습니다.",
        changelog_item_4: "Cyber Safety 문서를 반영해 일부 보안 작업이 `gpt-5.2`로 reroute될 수 있고 Trusted Access가 필요할 수 있음을 추가했습니다.",
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog/deprecations, API 모델 문서, Windows sandbox 발표, Dell enterprise 발표를 다시 확인하고 Platform/API·enterprise 항목은 Codex 기본 사용법과 분리했습니다.",
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn을 확인했습니다. Windows feature flag 관련 community claim은 공식 문서와 분리해 운영 메모로만 유지했고, 검증되지 않은 소셜 주장은 일반 문장으로 추가하지 않았습니다.",
        ...
        s9_intro: "ChatGPT 웹의 `chatgpt.com/codex`에서는 GitHub 환경을 연결하고 클라우드 코딩 작업을 실행할 수 있습니다. Remote connections는 별도 축입니다. ChatGPT 모바일 앱 preview에서는 active thread, 승인, model 변경, 새 작업 시작, screenshot/terminal output/diff/test result 확인을 이어갈 수 있지만, 실제 파일·credential·permission·plugin·browser setup·Computer Use·local tool은 연결된 host에 남습니다. 현재 공식 문서는 mobile setup을 awake/online macOS Codex App host에서 시작하라고 설명하고, 같은 account/workspace와 필요한 SSO/MFA/passkey를 요구할 수 있습니다. 또 다른 signed-in Codex App 기기가 같은 host를 제어할 수 있고, Codex App은 SSH config의 구체 host alias를 통해 SSH host 프로젝트를 열어 remote filesystem/shell에서 실행할 수 있습니다. Business/Enterprise에서는 admin/owner가 Remote Control을 workspace settings 또는 RBAC로 허용해야 할 수 있습니다. Windows phone 연결은 공식 문서상 coming soon이므로, Windows `remote_control = true`는 해당 빌드에서 보일 때 쓰는 별도 운영 메모로 읽으세요.",
        ...
        s17_intro_ref: '레퍼런스: <a href="https://developers.openai.com/codex/learn/best-practices/" target="_blank">Codex Best practices</a> · <a href="https://developers.openai.com/codex/use-cases/follow-goals" target="_blank">Follow goals</a> · <a href="https://developers.openai.com/api/docs/guides/prompt-guidance?model=gpt-5.5" target="_blank">GPT-5.5 Prompt guidance</a> · <a href="https://openai.com/index/introducing-gpt-5-5/" target="_blank">Introducing GPT-5.5</a>',
        ...
        pg_goals_t: "Goals preview는 장기 작업의 체크포인트 계약입니다",
        pg_goals_d: "공식 Follow goals 문서는 `/goal`을 큰 목표를 작게 쪼개고, 진행 상태를 기록하고, 멈춤 기준을 지키게 하는 preview 흐름으로 설명합니다. 한 번의 프롬프트보다 길지만 무한 backlog는 아닌 작업에 쓰고, 시작할 때 목표·완료 조건·검증 명령·중간 체크포인트·중단/승인 조건을 함께 적으세요.",
        pg_goals_f1: "`/goal`이 보이지 않으면 preview/experimental 기능 활성화가 필요한지 확인하세요. CLI에서는 설정의 `[features] goals = true` 같은 feature flag가 운영 메모가 될 수 있고, App 표면에서는 지원 상태가 다를 수 있습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 22 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. The May 22 check also records that Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 gets a usage boost through the same date. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        overview_update_3_title: "Quickstart, IDE, and mobile path tightened",
        overview_update_3_desc: "The top path still keeps App-first onboarding separate from IDE/CLI + Docs MCP professional workflows, but the remote-connection story was reworked more substantially today. The official docs separate phone control of an awake, online macOS Codex App host, continuation from another signed-in Codex App device, and Codex App projects that run on an SSH host. Remote access uses the connected host's files, credentials, permissions, plugins, browser setup, Computer Use, and local tools, and setup may require the same account/workspace plus SSO, MFA, or passkeys. Business/Enterprise admins may need to enable Remote Control in workspace settings or grant it through RBAC. Windows phone connection is still coming soon in the official wording, so the guide keeps the Windows `remote_control` note as a separate operating memo.",
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The May 18 Dell Technologies partnership post is treated as an enterprise deployment direction for hybrid and on-prem environments with Dell AI Data Platform / AI Factory, not as a general user feature to enable today. The May 13 `Building a safe, effective sandbox to enable Codex on Windows` post remains the engineering basis for the preferred `elevated` Windows sandbox.",
        overview_update_6_title: "Docs MCP, API changelog, and config rechecked",
        overview_update_6_desc: "Docs MCP and the config reference were rechecked around trusted project-scoped `.codex/config.toml`, the user-level requirement for machine-local provider/auth/profile/telemetry routing keys, shared CLI/IDE setup, the AGENTS.md steering snippet, `review_model`, granular approvals, `approvals_reviewer`, `default_permissions`, permissions profiles, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, `sqlite_home`, and hooks/plugin/skill controls. The OpenAI API changelog and deprecations docs were also checked: `gpt-5-codex`, `gpt-5.1-codex*`, and `gpt-5.2-codex` API snapshots are listed for July 23, 2026 shutdown with `gpt-5.5` as the substitute, which stays in the Platform/API lane rather than the Codex model-picker lane.",
        ...
        changelog_item_3: "Reworked Remote connections around mobile hosts, other Codex App devices, SSH hosts, connected-host state, admin Remote Control/RBAC, and the separate Windows operating memo.",
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, the OpenAI API changelog/deprecations, API model docs, the Windows sandbox post, and the Dell enterprise announcement; Platform/API and enterprise items stay separate from core Codex usage.",
        changelog_item_6: "The Chrome plugin checked X, Threads, and LinkedIn. Community Windows feature-flag claims were kept separate as operating notes, and unverified social claims were not promoted into general guidance.",
        ...
        s9_title: "Codex Web/Mobile (ChatGPT)",
        s9_intro: "Use Codex at `chatgpt.com/codex` for cloud-based coding tasks. Remote connections are a separate path: in the ChatGPT mobile app preview you can continue active threads, approve commands, change models, start work, and review screenshots, terminal output, diffs, and test results while files, credentials, permissions, plugins, browser setup, Computer Use, and local tools stay on the connected host. The official docs say mobile setup starts from an awake, online macOS Codex App host on the same account/workspace and may require SSO, MFA, or passkeys. Another signed-in Codex App device can also control that host, and the Codex App can open SSH-hosted projects through concrete SSH config aliases so threads run on the remote filesystem and shell. Business/Enterprise admins may need to enable Remote Control in workspace settings or grant it through RBAC. Windows phone connection is still coming soon in official wording, so treat `remote_control = true` under `[features]` as a separate Windows operating note for builds where that capability appears.",
        ...
        s17_intro_ref: 'References: <a href="https://developers.openai.com/codex/learn/best-practices/" target="_blank">Codex Best practices</a> · <a href="https://developers.openai.com/codex/use-cases/follow-goals" target="_blank">Follow goals</a> · <a href="https://developers.openai.com/api/docs/guides/prompt-guidance?model=gpt-5.5" target="_blank">GPT-5.5 Prompt guidance</a> · <a href="https://openai.com/index/introducing-gpt-5-5/" target="_blank">Introducing GPT-5.5</a>',
        ...
        pg_goals_t: "Goals preview is a checkpoint contract for longer work",
        pg_goals_d: "The official Follow goals guide frames `/goal` as a preview flow for breaking a larger objective into smaller steps, tracking progress, and respecting stop conditions. Use it for work that is bigger than one prompt but smaller than an open-ended backlog, and start with the objective, completion criteria, validation commands, intermediate checkpoints, and stop/approval rules.",
        pg_goals_f1: "If `/goal` is unavailable, check whether preview or experimental features need to be enabled. In CLI-oriented setups, `[features] goals = true` can be the operating note, while App support may differ by surface.",
