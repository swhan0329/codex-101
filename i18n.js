...
const translations = {
    ko: {
        ...
        overview_fact_6_desc: "`review_model`, `approval_policy.granular`, `approvals_reviewer`, top-level `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, `service_tier`, `personality`, `default_permissions`, `windows.sandbox`, `windows.sandbox_private_desktop`, `model_instructions_file`, `background_terminal_max_timeout`, `memories.disable_on_external_context`, `sqlite_home`, TUI 설정, protected paths, JSON schema, app connector 권한, `features.fast_mode`, `features.prevent_idle_sleep`, `features.network_proxy`, permissions profile, sandboxed network proxy/requirements, keyring credential storage, forced login/workspace, OpenTelemetry export, plugin 배포 계층, Bedrock provider 설정, 로컬 OS 샌드박스와 cloud split runtime 설명을 함께 최신 기준으로 맞췄습니다. 특히 project-scoped config는 trusted project에서만 로드되고 provider/auth/profile/telemetry 라우팅 키는 user-level config에 둬야 합니다. `experimental_instructions_file`은 `model_instructions_file`로 바꾸라는 deprecation 메모도 추가했습니다. OpenAI API changelog와 deprecations 문서는 Platform/API 개발자용 흐름으로 따로 읽어야 합니다. GPT-5.5는 API 최신 모델 가이드에서도 유지되며, 기본 reasoning effort medium과 image input detail 기본값 변화는 직접 API 앱 안내에만 반영했습니다.",
        ...
        changelog_item_4: "Config reference의 `background_terminal_max_timeout`, `features.fast_mode`, `features.prevent_idle_sleep`, `features.network_proxy`, `experimental_instructions_file` deprecation을 추가했습니다.",
        ...
        changelog_item_6: "`codex-x-tips-2026-06-08.md`의 계정별 X 크롤링 내용을 Chapter 20에 source-gated 패턴으로 정리했고, `/goal`, Automations, Skills, Subagents, Browser/Chrome, SDK는 공식 문서와 대조했습니다.",
        ...
        field_tips_intro: "2026-06-08 Chrome 로그인 세션으로 확인한 OpenAI Codex 관련 실무자들의 X 최근 글을 공식 문서와 대조해, 문서에 넣어도 되는 사용 습관만 추렸습니다.",
    ...
    en: {
        ...
        changelog_item_4: "Added config-reference drift: `background_terminal_max_timeout`, `features.fast_mode`, `features.prevent_idle_sleep`, `features.network_proxy`, and the `experimental_instructions_file` deprecation.",
        ...
        changelog_item_6: "Added the account-level X crawl from `codex-x-tips-2026-06-08.md` to Chapter 20 as source-gated practice patterns, and cross-checked `/goal`, Automations, Skills, Subagents, Browser/Chrome, and SDK against official docs.",
        ...
        field_tips_intro: "On June 8, 2026, I checked recent X posts from OpenAI Codex practitioners through the logged-in Chrome session, then cross-checked the patterns against official docs before keeping them here.",
