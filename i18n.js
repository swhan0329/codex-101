...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 5월 15일 기준으로 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex`의 Cloud/code review 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 특히 오늘도 `codex/models`가 `gpt-5.5`의 API Access를 true로 표시하는 반면 `codex/pricing`의 API Key 표는 아직 Not available로 두는 차이를 분리해 설명했습니다.",
        overview_update_2_title: "요금제·크레딧 구조 재정리",
        overview_update_2_desc: "개인 플랜 한도뿐 아니라 `GPT-5.5` 사용량 표, `Free $0`·`Go $8`·`Plus $20`·`Pro from $100`, Plus의 web/CLI/IDE/iOS 접근, Business·Enterprise의 credits 설명, 그리고 현재 `codex/pricing` 문서의 Plus / Pro 5x / Pro 20x / Business / API Key 구성을 다시 맞췄습니다. API-key 사용자는 실제 Codex 모델 선택기와 표준 API pricing을 같이 확인해야 한다는 주의점을 가격 섹션에서 더 또렷하게 분리했습니다.",
        overview_update_3_title: "Quickstart·IDE·Mobile 경로 정리",
        overview_update_3_desc: "Quickstart 기준으로 처음 사용자는 App 중심, 실무 사용자는 IDE/CLI + Docs MCP 중심으로 읽히도록 시작 경로를 유지하되, API 키 로그인 제한과 Windows/WSL 설명을 더 분명하게 다듬었습니다. 2026년 5월 14일 OpenAI 발표 기준으로 Codex는 ChatGPT 모바일 앱에서 iOS/Android preview로 롤아웃 중이며, Mac/Windows Codex 앱이나 원격 환경에서 실행 중인 작업을 휴대폰에서 이어 보고 승인하고 방향을 바꿀 수 있습니다. Windows는 `config.toml`의 `[features]` 아래 `remote_control = true`를 추가한 뒤 재시작하는 활성화 메모를 함께 넣었습니다.",
        ...
        overview_update_6_desc: "Docs MCP와 config reference를 다시 확인해 shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `plan_mode_reasoning_effort`, granular approvals, `web_search`, permissions profile, keyring credential storage, forced login/workspace, shell environment policy, project root markers, TUI 설정, OpenTelemetry, apps/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. OpenAI API changelog의 2026년 5월 항목도 별도로 확인해 5월 12일 DALL-E 2/3 snapshot 및 Realtime API Beta 제거, Responses API web search의 `return_token_budget`, Realtime 2/Realtime Translate/Realtime Whisper, OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, Admin API SDK 지원은 Codex 기본 사용법이 아니라 Platform/API 개발자 참고사항으로 분리했습니다.",
        ...
        overview_fact_2_desc: "오늘 기준 `Introducing GPT-5.5`, 최신 `codex/models`, API Models 문서를 함께 보면 `gpt-5.5`가 복잡한 코딩·computer use·knowledge work·research workflow의 최우선 추천 모델이라는 점이 유지됩니다. 다만 오늘 문서 정리에서는 “Codex 안에서 고르는 모델”, “API-key authentication으로 Codex를 쓰는 경우”, “직접 API에서 호출 가능한 모델”을 명확히 분리했습니다. API 자체에서는 `gpt-5.5`와 `gpt-5.5-pro`를 별도 모델로 사용할 수 있고, Codex API-key 사용자는 모델 페이지와 가격표의 차이를 함께 확인해야 합니다. `gpt-5.4`는 rollout 계정이나 비용/속도 기준에서 여전히 강력한 대체 선택지이고, `gpt-5.4-mini`는 빠른 작업과 subagent용 효율 모델로 남아 있습니다.",
        overview_fact_3_title: "플랜/팀 요금제 보강",
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. `codex/pricing`은 오늘도 Plus / Pro 5x / Pro 20x / Business / API Key 표와 Plus 로컬 메시지 범위(`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150)를 유지합니다. Plus는 web, CLI, IDE extension, iOS 접근을 함께 표시합니다. 다만 API-key 관점에서는 `codex/models`와 `codex/pricing`의 표현이 다르므로, 실무 문서는 사용량 표, token credit 표, usage dashboard, 실제 모델 선택기, API 모델 가격표를 섞지 말고 함께 확인하도록 고쳤습니다.",
        ...
        s1_refs: '출처: <a href="https://developers.openai.com/codex/" target="_blank">Codex overview</a> · <a href="https://developers.openai.com/codex/models/" target="_blank">Codex models</a> · <a href="https://developers.openai.com/api/docs/models" target="_blank">OpenAI API Models</a> · <a href="https://developers.openai.com/api/docs/changelog" target="_blank">OpenAI API changelog</a> · <a href="https://openai.com/index/introducing-gpt-5-5/" target="_blank">Introducing GPT-5.5</a>',
        ...
        s9_refs: '출처: <a href="https://developers.openai.com/codex/quickstart/" target="_blank">Quickstart</a> · <a href="https://developers.openai.com/codex/cloud" target="_blank">Codex cloud</a> · <a href="https://developers.openai.com/codex/integrations/github" target="_blank">GitHub integration</a> · <a href="https://openai.com/index/work-with-codex-from-anywhere/" target="_blank">Work with Codex from anywhere</a>',
        ...
        changelog_item_2: "가격 설명에서 Go/iOS 표기, credit 적용 범위, API-key 사용 시 실제 모델 선택기와 직접 API 모델 가격을 함께 확인해야 한다는 주의점을 맞췄습니다.",
        changelog_item_3: "Quickstart·IDE·Windows 문서와 5월 14일 mobile preview 발표를 대조해 App-first 시작 경로, JetBrains 지원, Microsoft Store / `winget`, WSL2, iOS/Android 조율 표면, Windows `remote_control` 활성화 절차를 정리했습니다.",
        changelog_item_4: "Cyber Safety 문서를 반영해 일부 보안 작업이 `gpt-5.2`로 reroute될 수 있고 Trusted Access가 필요할 수 있음을 추가했습니다.",
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog를 다시 확인하고 DALL-E/Realtime 제거 등 Platform/API 신규 항목은 Codex 기본 사용법과 분리했습니다.",
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn 탭을 확인했지만 도메인 읽기가 정책상 차단되어 새 소셜 팁은 추가하지 않았습니다.",
        ...
        s1_intro: "OpenAI Codex는 소프트웨어 개발을 도와주는 코딩 에이전트입니다. 쉽게 말하면, 코드를 읽고 고치고 테스트까지 같이 해주는 “개발 보조”에 가깝습니다. 최신 공식 문서는 Codex를 앱, IDE, CLI, 웹/iOS, SDK 기반 자동화에서 사용할 수 있다고 설명하며, 지금은 모델 선택기에서 `gpt-5.5`가 보이면 대부분의 작업을 거기서 시작하라고 권장합니다. 오늘 기준 `codex/models`는 `gpt-5.5`에 API Access를 표시하지만 `codex/pricing`의 API Key 표는 아직 Not available로 남아 있으므로, API-key 사용자는 실제 모델 선택기와 표준 API pricing을 함께 확인해야 합니다. API에서는 공식 Models 문서 기준으로 `gpt-5.5`를 사용할 수 있고, 더 높은 정확도가 필요한 작업에는 `gpt-5.5-pro`도 Responses API와 Batch API에서 사용할 수 있습니다. 빠른 로컬 작업에는 `gpt-5.4-mini`, Codex Cloud와 클라우드 리뷰에는 `gpt-5.3-codex`, 거의 즉각적인 반복에는 `gpt-5.3-codex-spark`, 대안 모델로는 `gpt-5.2`를 보면 됩니다.",
        ...
        s9_intro: "ChatGPT 웹의 `chatgpt.com/codex`에서는 GitHub 환경을 연결하고 클라우드 코딩 작업을 실행할 수 있습니다. 2026년 5월 14일 OpenAI 발표 기준으로 Codex는 ChatGPT 모바일 앱에서도 iOS/Android preview로 롤아웃 중입니다. 휴대폰은 Codex가 실행 중인 Mac, Windows, devbox, managed remote environment에 연결해 active thread, 승인, model 변경, 새 작업 시작, screenshot/terminal output/diff/test result 확인을 이어가는 조율 표면입니다. 파일, credential, permission, local setup은 원래 머신에 남습니다. Windows에서 mobile remote control이 보이지 않으면 `~/.codex/config.toml`의 `[features]` 아래 `remote_control = true`를 추가하고 저장 후 Codex를 재시작하세요.",
        ...
        s13_c1_t: "무엇을 고정할까",
        s13_c1_d: "모델, model_provider, review_model, reasoning effort, sandbox mode, approval policy, web_search, profile, MCP 서버처럼 세션마다 흔들리면 품질 차이가 큰 항목부터 기본값으로 고정하세요. AWS limited preview처럼 Bedrock 경로를 쓰는 팀은 provider 설정도 운영 표준에 포함해야 합니다. 필요하면 Windows 전용 `windows.sandbox`, `windows.sandbox_private_desktop`, mobile 연동용 `[features].remote_control`도 함께 고정하세요.",
        s13_c2_t: "권한은 좁게 시작",
        ...
        s18_field_credit: "이번 실행에서는 Chrome 연결과 기존 X, Threads, LinkedIn 탭 확인은 성공했지만, 브라우저 정책이 해당 도메인 읽기를 차단했습니다. 그래서 기존 공개 출처·공식 문서·저장소를 유지했고, Chrome에서 새로 검증하지 못한 소셜 팁은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 15 run started by re-checking `codex/models`, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        overview_update_3_title: "Quickstart, IDE, and mobile path tightened",
        overview_update_3_desc: "The top path still keeps App-first onboarding separate from IDE/CLI + Docs MCP professional workflows in Quickstart, but now also spells out API-key sign-in limits and the Windows/WSL path more clearly. The May 14 OpenAI announcement adds Codex in the ChatGPT mobile app as an iOS/Android preview: it lets you follow, approve, redirect, change models, and start work from the phone while Codex keeps running on a Mac, Windows machine, or remote environment. For Windows, the guide now includes the `config.toml` enablement note: add `remote_control = true` under `[features]`, save, and restart Codex.",
        ...
        overview_update_6_desc: "Docs MCP and the config reference were rechecked around shared CLI/IDE setup, the AGENTS.md steering snippet, `review_model`, `plan_mode_reasoning_effort`, granular approvals, `web_search`, permissions profiles, keyring credential storage, forced login/workspace, shell environment policy, project root markers, TUI settings, telemetry, and app/plugin/skill controls. The May 2026 OpenAI API changelog was also checked separately: the May 12 removal of DALL-E 2/3 snapshots and the Realtime API Beta, Responses API web search `return_token_budget`, Realtime 2 / Realtime Translate / Realtime Whisper, the OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, and Admin API SDK support are Platform/API developer updates, not Codex model-picker changes.",
        ...
        overview_fact_2_desc: "The latest model story is now split more clearly. `gpt-5.5` is still the top recommendation for complex coding, computer use, knowledge work, and research workflows in Codex. In this refresh, the guide separates the Codex model picker, API-key authentication for Codex, and direct API model calls. The API itself exposes `gpt-5.5` and `gpt-5.5-pro` as separate models, while API-key Codex users should compare the models page, pricing table, and actual product picker. `gpt-5.4` still matters for rollout, cost, or latency tradeoffs, while `gpt-5.4-mini` remains the efficient choice for lighter work and subagents.",
        overview_fact_3_title: "Plans and team pricing clarified",
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / API Key tables and the published Plus local-message ranges (`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150). Plus now explicitly includes web, CLI, IDE extension, and iOS access. The important split is that API-key availability is described differently across `codex/models` and `codex/pricing`; direct OpenAI API access to `gpt-5.5` and `gpt-5.5-pro` belongs to the separate API Models/Pricing docs. The guide tells readers to check usage tables, token credit rates, the usage dashboard, the actual picker, and API model pricing without collapsing them into one bucket.",
        ...
        overview_fact_6_desc: "The guide now keeps `review_model`, `plan_mode_reasoning_effort`, granular approvals, `approvals_reviewer`, top-level `web_search`, `tools.web_search`, `tools.view_image`, `service_tier`, `personality`, `default_permissions`, Windows sandbox keys, `model_instructions_file`, `memories.disable_on_external_context`, `sqlite_home`, shell environment policy, project root markers, TUI settings, protected paths, JSON schema, app-connector controls, feature flags, permissions profiles, keyring credential storage, forced login/workspace controls, OpenTelemetry export, plugin packaging, and the split cloud runtime aligned with current docs. The May API changelog belongs to the Platform/API developer lane: the May 12 removal of DALL-E 2/3 snapshots and the Realtime API Beta, Responses API web search `return_token_budget`, Realtime 2, the OpenAI Developers plugin, TypeScript Agents SDK, `chat-latest`, and Admin API SDK support should not be collapsed into Codex model guidance. This run keeps May 8 `Running Codex safely at OpenAI` and the May 7 Codex Security plugin as security/operations evidence, not changes to the Codex model-picker order.",
        ...
        overview_source_32: "GPT-5.5-Cyber and Codex Security",
        overview_source_33: "OpenAI API Changelog",
        s1_refs: 'Sources: <a href="https://developers.openai.com/codex/" target="_blank">Codex overview</a> · <a href="https://developers.openai.com/codex/models/" target="_blank">Codex models</a> · <a href="https://developers.openai.com/api/docs/models" target="_blank">OpenAI API Models</a> · <a href="https://developers.openai.com/api/docs/changelog" target="_blank">OpenAI API changelog</a> · <a href="https://openai.com/index/introducing-gpt-5-5/" target="_blank">Introducing GPT-5.5</a>',
        ...
        s9_refs: 'Sources: <a href="https://developers.openai.com/codex/quickstart/" target="_blank">Quickstart</a> · <a href="https://developers.openai.com/codex/cloud" target="_blank">Codex cloud</a> · <a href="https://developers.openai.com/codex/integrations/github" target="_blank">GitHub integration</a> · <a href="https://openai.com/index/work-with-codex-from-anywhere/" target="_blank">Work with Codex from anywhere</a>',
        ...
        changelog_item_2: "Realigned Go/iOS plan wording, credit scope, and the warning that API-key users should verify the actual picker plus standard API model pricing.",
        changelog_item_3: "Rechecked Quickstart, IDE, Windows docs, and the May 14 mobile preview announcement so the App-first path, JetBrains support, Microsoft Store / `winget`, WSL2, iOS/Android coordination surface, and Windows `remote_control` enablement read clearly.",
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, and the OpenAI API changelog; Platform/API updates, including DALL-E/Realtime removals, stay separate from core Codex usage.",
        changelog_item_6: "The Chrome plugin found X, Threads, and LinkedIn tabs, but policy blocked reading those domains, so no fresh social tips were added.",
        ...
        s9_title: "Codex Web/Mobile (ChatGPT)",
        s9_intro: "Use Codex at `chatgpt.com/codex` for cloud-based coding tasks. You can connect a GitHub environment, delegate via `@codex` in a pull request comment, monitor logs, review diffs, and open PRs from the browser. The May 14 OpenAI announcement also adds Codex in the ChatGPT mobile app as an iOS/Android preview. The phone connects to machines where Codex is already running, including a Mac, Windows machine, devbox, or managed remote environment, so you can continue active threads, approve commands, change models, start new work, and review screenshots, terminal output, diffs, and test results. Files, credentials, permissions, and local setup stay on the machine where Codex operates. If mobile remote control is not visible on Windows, add `remote_control = true` under `[features]` in `~/.codex/config.toml`, save, and restart Codex.",
        ...
        s13_c1_d: "Start by fixing the defaults that most affect consistency: model choice, model_provider, review_model, reasoning effort, sandbox mode, approval policy, web_search, profiles, and MCP setup. Teams using the AWS limited-preview Bedrock path should treat provider configuration as part of the rollout standard. Also set `windows.sandbox`, `windows.sandbox_private_desktop`, and `[features].remote_control` if you work natively on Windows and want mobile coordination.",
        ...
        s18_field_credit: "Chrome connected successfully in this run and found existing X, Threads, and LinkedIn tabs, but the active browser policy blocked reading those domains. I kept the existing public sources, official docs, and repositories, and did not add fresh social tips that could not be verified in Chrome.",
