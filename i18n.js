...
const translations = {
    ko: {
        ...
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn을 확인했지만 검증 가능한 현재 Codex 팁 본문을 얻지 못해 새 소셜 팁은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 18 run started by re-checking `codex/models`, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        ...
        changelog_item_6: "The Chrome plugin checked X, Threads, and LinkedIn, but no verifiable current Codex tip text was available, so no fresh social tips were added.",
        ...
        s18_field_credit: "Chrome connected successfully in this run. X returned blank readable content, and Threads/LinkedIn exposed only search or navigation chrome rather than usable current Codex posts. I kept the existing public sources, official docs, and repositories, and did not add fresh social tips that could not be verified in Chrome.",
