...
const translations = {
    ko: {
        ...
        overview_update_4_title: "Cyber Safety 주의점 보강",
        overview_update_4_desc: "Cyber Safety 문서를 다시 확인해 `gpt-5.3-codex`의 고급 cyber capability, 일부 보안 관련 요청이 `gpt-5.2`로 reroute될 수 있는 점, legitimate defensive work를 위한 Trusted Access 경로를 보안/운영 설명에 추가했습니다.",
        ...
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명합니다. `codex/pricing`은 오늘도 Plus / Pro 5x / Pro 20x / Business / API Key 표와 Plus 로컬 메시지 범위(`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150)를 유지합니다. Plus는 web, CLI, IDE extension, iOS 접근을 함께 표시합니다. 다만 Codex API-key 로그인은 `gpt-5.5`가 제공되지 않고, 직접 OpenAI API의 `gpt-5.5`/`gpt-5.5-pro` 모델 접근은 별도 API 문서 기준으로 봐야 합니다. 따라서 실무 문서는 사용량 표, token credit 표, usage dashboard, API 모델 표를 섞지 말고 함께 확인하도록 고쳤습니다.",
        ...
        changelog_item_2: "가격 설명에서 Go/iOS 표기, credit 적용 범위, Codex API-key 로그인의 `gpt-5.5` 미지원, 직접 API 모델 표 분리를 다시 맞췄습니다.",
        ...
        changelog_item_4: "Cyber Safety 문서를 반영해 일부 보안 작업이 `gpt-5.2`로 reroute될 수 있고 Trusted Access가 필요할 수 있음을 추가했습니다.",
        changelog_item_5: "Docs MCP, best practices, config 문서를 다시 확인하고 granular approvals, app/plugin 제어, permissions profile, telemetry를 재점검했습니다.",
        changelog_item_6: "Chrome은 연결됐지만 Google, X, Threads, LinkedIn 접근이 정책상 차단되어 새 소셜 팁은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 12 run started by re-checking `codex/models`, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The login-path distinction stays explicit up front: inside Codex, `gpt-5.5` currently appears only with ChatGPT sign-in, not API-key authentication.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. This run also makes the split clearer: Codex API-key sign-in does not expose `gpt-5.5`, while direct OpenAI API model access is documented separately.",
        ...
        overview_update_4_title: "Cyber Safety caveat added",
        overview_update_4_desc: "The Cyber Safety docs were rechecked, and the guide now calls out GPT-5.3-Codex's advanced cyber capability, possible rerouting of some security-related work to `gpt-5.2`, and Trusted Access for legitimate defensive workflows.",
        ...
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / API Key tables and the published Plus local-message ranges (`gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150). Plus now explicitly includes web, CLI, IDE extension, and iOS access. The important split is that Codex API-key sign-in does not expose `gpt-5.5`; direct OpenAI API access to `gpt-5.5` and `gpt-5.5-pro` belongs to the separate API Models/Pricing docs. The guide tells readers to check usage tables, token credit rates, the usage dashboard, and API model pricing without collapsing them into one bucket.",
        ...
        changelog_item_2: "Realigned Go/iOS plan wording, credit scope, Codex API-key `gpt-5.5` limits, and the separate direct API model table.",
        ...
        changelog_item_4: "Added the Cyber Safety caveat that some security work can be rerouted to `gpt-5.2` and may need Trusted Access.",
        changelog_item_5: "Rechecked Docs MCP, best practices, and config docs around granular approvals, app/plugin controls, permission profiles, and telemetry.",
        changelog_item_6: "Chrome connected, but browser policy blocked Google, X, Threads, and LinkedIn, so no fresh social tips were added.",
