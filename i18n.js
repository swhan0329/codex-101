...
const translations = {
    ko: {
        ...
        changelog_item_5: "Docs MCP, config, app/settings, OpenAI API changelog/deprecations, Apps SDK changelog, API 모델 문서, Windows sandbox 발표, Braintrust/How OpenAI uses Codex, Dell/Gartner enterprise 발표를 다시 확인했습니다.",
        changelog_item_6: "Chrome 소셜 확인에서 X/Threads/LinkedIn을 읽었지만 사용량 리셋 등 검증되지 않은 claim은 추가하지 않았고, 공식 문서로 검증된 운영 패턴만 반영했습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome에서 X/Threads/LinkedIn 본문을 읽을 수 있었지만, X의 사용량 리셋 claim처럼 공식 문서로 재확인되지 않은 내용은 추가하지 않았습니다. 공식 릴리스 노트와 문서로 재확인된 Appshots, Profile, Windows Computer Use, 권한 확인, 출력 계약, 도구 사용 규칙, 완료 기준, OpenAI Developers plugin 활용 메모만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The June 1 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. On June 1, the official docs and Help Center still describe the Pro $100 2x promo and Pro $200 higher 25x five-hour Codex limits as ending on May 31, 2026, so the guide now tells readers to verify current limits in the Codex usage dashboard and plan banner. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The May 29 Braintrust customer story and official `How OpenAI uses Codex` resource are reflected as workflow patterns: customer requests to preview branches, Ask Mode, issue-shaped prompts, environment iteration, AGENTS.md, and Best-of-N. The May 27 Tax AI/Codex engineering post remains a self-improving agent operating pattern built from practitioner corrections, production traces, eval targets, bounded task environments, and human review. The May 22 Gartner and May 18 Dell posts remain enterprise governance and hybrid/on-prem deployment context.",
        ...
        changelog_item_5: "Rechecked Docs MCP, config, app/settings, the OpenAI API changelog/deprecations, Apps SDK changelog, API model docs, the Windows sandbox post, Braintrust/How OpenAI uses Codex, and Dell/Gartner enterprise announcements.",
        changelog_item_6: "Chrome social verification could read X, Threads, and LinkedIn, but unverified usage-reset claims were not added; only officially corroborated operating patterns were reflected.",
        ...
        s18_field_credit: "Chrome social verification could read X, Threads, and LinkedIn, but unverified claims such as usage-limit resets were not added. Only officially corroborated Appshots, Profile, Windows Computer Use, permissions, output contracts, tool-use rules, done criteria, and OpenAI Developers plugin notes were tied back to the guide.",
