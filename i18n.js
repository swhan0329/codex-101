...
const translations = {
    ko: {
        ...
        overview_update_6_desc: "Docs MCP와 config reference를 다시 확인해 shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `model_reasoning_effort`, granular approvals, `approvals_reviewer`, `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, permissions profile, keyring credential storage, forced login/workspace, `sqlite_home`, TUI 설정, OpenTelemetry, apps/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. OpenAI API changelog의 2026년 5월 항목도 별도로 확인해 5월 12일 DALL-E 2/3 snapshot 및 Realtime API Beta 제거, Responses API web search의 `return_token_budget`, Realtime 2/Realtime Translate/Realtime Whisper, OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, Admin API SDK 지원은 Codex 기본 사용법이 아니라 Platform/API 개발자 참고사항으로 분리했습니다.",
        ...
        s18_field_credit: "이번 실행에서는 Chrome 연결은 성공했지만, 브라우저 정책이 X, Threads, LinkedIn 도메인 열기를 차단했습니다. 그래서 기존 공개 출처·공식 문서·저장소를 유지했고, Chrome에서 새로 검증하지 못한 소셜 팁은 추가하지 않았습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 16 run started by re-checking `codex/models`, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        ...
        s18_field_credit: "Chrome connected successfully in this run, but the active browser policy blocked opening X, Threads, and LinkedIn. I kept the existing public sources, official docs, and repositories, and did not add fresh social tips that could not be verified in Chrome.",
