...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 5월 13일 기준으로 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex`의 Cloud/code review 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 특히 오늘은 `codex/models`가 `gpt-5.5`의 API Access를 true로 표시하는 반면 `codex/pricing`의 API Key 표는 아직 Not available로 두는 차이를 분리해 설명했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 13 run started by re-checking `codex/models`, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key drift is API-key nuance: `codex/models` now marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
