...
const translations = {
    ko: {
        ...
        overview_update_1_desc: "`codex/models`를 2026년 6월 15일 기준으로 가장 먼저 다시 확인했고, `gpt-5.5` 최우선 추천, `gpt-5.4` 대체 선택지, `gpt-5.4-mini`의 빠른 로컬/subagent 역할, `gpt-5.3-codex-spark`의 Pro research preview 역할이 유지되는지 대조했습니다. 현재 models 문서는 계속 `gpt-5.2`와 `gpt-5.3-codex`를 ChatGPT 로그인 기준 Codex에서는 deprecated라고 명시합니다. API-key authentication을 쓰는 workflow는 API models page에서 별도로 확인해야 하며, 임의 provider에서 Chat Completions API를 쓰는 Codex 경로는 future release에서 제거될 예정인 deprecated 지원입니다. 오늘 새로 반영한 본문 변화는 VB의 Codex Mobile control-plane 팁을 공식 Remote connections 문서 기준으로 검증해 소셜 팁 섹션에 추가한 것입니다.",
        overview_update_2_title: "요금제·크레딧 구조 재정리",
        overview_update_2_desc: "개인 플랜 한도뿐 아니라 GPT-5.5 pricing scope를 다시 나눴습니다. Plus/Pro/Business 사용량 표에는 GPT-5.5 local-message 범위가 보이고, API Key 사용량 표도 이제 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. `Free $0`·`Go $8`·`Plus $20`·`Pro from $100`, Plus의 web/CLI/IDE/iOS 접근, Business·Enterprise의 credits 설명, 그리고 현재 `codex/pricing` 문서의 Plus / Pro 5x / Pro 20x / Business / API Key 구성도 다시 맞췄습니다. 가격표에는 Sites가 Business/Enterprise에서 available, Plus/Pro/API Key에서는 unavailable로 표시되고, Sites FAQ는 preview 기간 동안 무료라고 설명합니다. 2026년 6월 11일부터 6월 24일까지 eligible Plus/Pro 초대는 첫 Codex 메시지 이후 양쪽에 30일짜리 banked rate-limit reset을 줄 수 있으므로, 초대 조건과 만료일은 dialog/current terms에서 확인해야 합니다. ChatGPT for Excel 같은 agentic feature도 유효 시점 이후 Codex usage limits를 공유할 수 있으므로, usage dashboard와 `/status`를 함께 확인하라고 정리했습니다.",
        ...
        overview_update_4_desc: "2026년 6월 11일 Codex app 26.609 릴리스는 Plus/Pro rate-limit reset banking, Developer mode의 Browser/CDP 접근, App composer의 `/init`, Enterprise Computer Use 확대, Windows Computer Use per-app access control을 추가했습니다. 이 항목들은 켜는 위치와 권한 영향이 있으므로 단순 기능 홍보가 아니라 사용 전 확인할 설정으로 분리했습니다. May 29 Windows Computer Use/remote control, May 21 Goal mode/Appshots/locked computer use, enterprise governance/RBAC/auditability도 계속 별도 맥락으로 유지했습니다.",
        overview_update_5_title: "관련 발표 전체 재점검",
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 6월 11일 `OpenAI to acquire Ona` 발표는 closing 전이며 규제 승인 조건이 있으므로, secure persistent enterprise execution 방향으로만 분리했습니다. OpenAI는 Ona 기술이 고객 cloud environment 안에서 agent가 더 오래 지속되는 작업을 하도록 돕는다고 설명하지만, 이를 오늘 켜진 일반 사용자 기능이나 feature flag로 쓰지 않았습니다. 6월 1일 AWS GA, 6월 10일 Oracle cloud commitment, 6월 11일 black-hole simulation 글도 각각 enterprise provider/procurement/research workflow 맥락으로만 반영했고, 새 모델 순서, 일반 사용자 플랜, preview/GA, activation 절차처럼 단정하지 않았습니다.",
        ...
        overview_fact_3_desc: "Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고, 데스크톱에서는 App 설치 경로를 먼저 보여주되 IDE extension, CLI, Cloud도 정식 시작 경로로 유지합니다. `codex/pricing`은 Plus / Pro 5x / Pro 20x / Business / Enterprise & Edu / API Key 표를 유지하고, Plus 설명에는 GPT-5.5, GPT-5.4, GPT-5.4 mini가 표시됩니다. 사용량 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini 중심이고, Pro에는 `gpt-5.3-codex-spark` research preview가 별도 혜택으로 남아 있습니다. 반면 `gpt-5.3-codex` 자체는 models 문서에서 ChatGPT 로그인 기준 deprecated로 표시하므로, 가격 한도와 추천 모델 상태는 서로 다른 범위로 읽어야 합니다. Sites는 현재 Business/Enterprise preview로 표시되고, pricing FAQ는 preview 기간 동안 무료라고 설명합니다. 실무 문서는 사용량 표, token credit 표, usage dashboard, App `Settings > Profile`의 lifetime tokens/token activity, 실제 모델 선택기, API 모델 가격표를 섞지 말고 함께 확인하도록 고쳤습니다.",
        ...
        changelog_item_1: "`codex/models`를 먼저 재확인했고, `gpt-5.5` 우선 추천과 `gpt-5.2`/`gpt-5.3-codex`의 ChatGPT 인증 Codex deprecated 상태가 유지됨을 반영했습니다.",
        changelog_item_2: "Pricing의 API Key 표가 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시하는 최신 범위와 6월 11-24일 Plus/Pro 초대 reset banking 조건을 반영했습니다.",
        changelog_item_3: "Quickstart의 standalone installer, App/IDE/CLI/Cloud 선택 구조, 원격 연결 주의점을 다시 맞췄습니다.",
        ...
        changelog_item_5: "OpenAI Developers MCP로 GPT-5.5 최신 모델, API changelog, Apps SDK 관련 표면을 다시 확인했고 Codex와 Platform/API 범위를 분리했습니다.",
        changelog_item_6: "로그인된 Chrome으로 X, Threads, LinkedIn의 사람별 최신 글을 다시 확인하고, 공식 Remote connections 문서로 대조 가능한 Codex Mobile control-plane 팁만 섹션 20과 별도 리서치 노트에 반영했습니다.",
        ...
        s3_rec_note: "💡 최신 `codex/models` 기준 공식 가이드는 모델 선택기에서 `gpt-5.5`가 보이면 대부분의 작업을 거기서 시작하라고 권장합니다. 오늘은 같은 모델 페이지가 API Access를 true로 표시하고 pricing의 Plus/Pro/Business 사용량 표도 GPT-5.5 local-message 범위를 보여주며, API Key 사용량 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. API-key 사용자는 실제 Codex 모델 선택기와 표준 API pricing을 같이 확인하세요. 비용/속도 균형이 중요하면 `gpt-5.4`를 계속 사용하면 됩니다. API에서는 `gpt-5.5`와 `gpt-5.5-pro`를 사용할 수 있으며, Pro는 더 높은 정확도가 필요한 Responses API/Batch API 작업에 적합합니다. 더 빠르고 가벼운 코딩 작업이나 subagent에는 `gpt-5.4-mini`, 거의 즉각적인 반복에는 ChatGPT Pro research preview인 `gpt-5.3-codex-spark`를 검토하세요. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 로그인 Codex 기준 deprecated입니다.",
        ...
        field_tips_intro: "2026-06-15 로그인된 Chrome 세션으로 X, Threads, LinkedIn의 최신 글을 다시 확인했습니다. 소셜 글은 “현장 신호”로만 쓰고, 제품 상태와 활성화 방법은 공식 Codex 문서와 OpenAI 발표로 대조했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The June 15 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and that `gpt-5.4-mini` and `gpt-5.3-codex-spark` keep their fast local/subagent and Pro research-preview roles. The models page still lists `gpt-5.2` and `gpt-5.3-codex` as deprecated in Codex when you sign in with ChatGPT. API-key workflows should verify API models separately, and the arbitrary-provider Chat Completions path remains a deprecated Codex support path that will be removed in a future release. The only new field tip promoted today is VB's Codex Mobile control-plane pattern, corroborated against the official Remote connections docs.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps GPT-5.5 pricing scope split cleanly: Plus/Pro/Business usage-limit tables show GPT-5.5 local-message ranges, and the API Key usage table now lists GPT-5.5/GPT-5.4/GPT-5.4 mini as usage-based. It also keeps `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. Pricing still lists Sites as available for Business/Enterprise and unavailable for Plus/Pro/API Key, while the Sites FAQ says it is free during preview. From June 11 through June 24, 2026, eligible Plus and Pro invitations can grant both users a banked rate-limit reset after the recipient sends their first Codex message, so users should check the invite dialog and current terms for eligibility and expiry. Agentic features such as ChatGPT for Excel can share Codex usage limits once pricing for those features is effective.",
        ...
        overview_update_4_desc: "The June 11 Codex app 26.609 release adds Plus/Pro rate-limit reset banking, Developer mode for Browser/CDP access, `/init` in the app composer, expanded Enterprise Computer Use, and per-app access controls for Computer Use on Windows. These are settings and permission-impacting changes, so the guide treats them as things to review before enabling, not just feature announcements. The May 29 Windows Computer Use/remote-control notes and May 21 Goal mode, Appshots, locked computer use, governance, RBAC, and auditability context remain separate.",
        overview_update_5_title: "Broader launch-post sweep",
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The June 11 `OpenAI to acquire Ona` announcement is still subject to closing conditions and required regulatory approvals, so the guide treats it only as direction for secure, persistent enterprise execution. OpenAI says Ona's technology can help agents keep working in customer cloud environments over longer horizons, but I did not describe it as an enabled user feature or feature flag. The June 1 AWS GA post, June 10 Oracle cloud commitment, and June 11 black-hole simulation article stay separated as enterprise provider, procurement, and research workflow context rather than model-order, consumer-plan, preview/GA, or activation changes.",
        ...
        overview_fact_3_desc: "Quickstart still says every ChatGPT plan includes Codex and now puts the App path first on desktop platforms while keeping IDE extension, CLI, and Cloud as official choices. The current `codex/pricing` page keeps the Plus / Pro 5x / Pro 20x / Business / Enterprise & Edu / API Key tables. Plus lists latest models such as GPT-5.5, GPT-5.4, and GPT-5.4 mini, and usage tables now center on GPT-5.5/GPT-5.4/GPT-5.4 mini. Pro still includes `gpt-5.3-codex-spark` as a research-preview benefit, but `gpt-5.3-codex` itself is deprecated for ChatGPT sign-in on the models page, so the guide separates pricing limits from current model recommendations. Sites is listed as a Business/Enterprise preview, and the pricing FAQ says it is free during preview. The important split is that API-key availability is described differently across `codex/models` and `codex/pricing`; direct OpenAI API access to `gpt-5.5` and `gpt-5.5-pro` belongs to the separate API Models/Pricing docs.",
        ...
        changelog_item_1: "Rechecked `codex/models` first and kept `gpt-5.5` as the top recommendation while preserving the deprecated status for `gpt-5.2` and `gpt-5.3-codex` in ChatGPT-authenticated Codex.",
        changelog_item_2: "Reflected the Pricing API Key table's current usage-based GPT-5.5/GPT-5.4/GPT-5.4 mini scope and the June 11-24 Plus/Pro invitation reset-banking terms.",
        changelog_item_3: "Aligned Quickstart with the standalone installer, App/IDE/CLI/Cloud choices, and remote-connection cautions.",
        ...
        changelog_item_5: "Rechecked GPT-5.5 latest-model guidance, API changelog, and Apps SDK-adjacent surfaces through OpenAI Developers MCP while keeping Platform/API content separate from Codex setup.",
        changelog_item_6: "Rechecked recent posts from individual people on X, Threads, and LinkedIn through logged-in Chrome, then added only the Codex Mobile control-plane tip corroborated by official Remote connections docs.",
        ...
        s3_rec_note: "💡 The latest `codex/models` guidance says to start with `gpt-5.5` when it appears in your picker. Today that page marks GPT-5.5 API Access as true, pricing shows GPT-5.5 local-message ranges for Plus/Pro/Business, and the API Key usage table lists GPT-5.5/GPT-5.4/GPT-5.4 mini as usage-based. API-key users should verify the actual Codex picker and standard API pricing. If cost/latency matters more, keep using `gpt-5.4`. In the API, `gpt-5.5` and `gpt-5.5-pro` are available, with Pro aimed at higher-accuracy Responses API and Batch API work. Use `gpt-5.4-mini` for faster lighter tasks or subagents, and `gpt-5.3-codex-spark` for near-instant ChatGPT Pro research-preview iteration. `gpt-5.2` and `gpt-5.3-codex` are deprecated for Codex when signing in with ChatGPT.",
        computer_use_kicker: "Computer use",
        ...
        field_tips_intro: "On June 15, 2026, I rechecked recent posts on X, Threads, and LinkedIn through the logged-in Chrome session. Social posts are treated as field signals, while product status and enablement steps are checked against official Codex docs and OpenAI announcements.",
