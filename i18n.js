...
const translations = {
    ko: {
        ...
        s1_intro: "OpenAI Codex는 코드를 읽고, 고치고, 테스트하고, 결과를 설명하는 코딩 에이전트입니다. 자동완성보다 한 단계 더 나아가, 개발자가 맡긴 작업을 실제 변경과 검증까지 이어가는 팀원에 가깝습니다.",
        s1_philosophy: '"단순 코드 생성 → 소프트웨어 엔지니어 팀원"',
        s1_desc: "이 섹션에서는 Codex를 어디서 실행할 수 있는지, 그리고 각 표면이 어떤 작업에 어울리는지 먼저 잡고 갑니다.",
    ...
    en: {
        ...
        s1_intro: "OpenAI Codex is a coding agent that can read code, make changes, run checks, and explain the result. Think of it less as autocomplete and more as a software engineering teammate that helps carry a task through implementation and verification.",
        s1_philosophy: '"From code generation → software engineer teammate"',
        s1_desc: "This section first maps where Codex runs and which surface fits each kind of work.",
