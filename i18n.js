...
const translations = {
    ko: {
        ...
        changelog_item_3: "Quickstart·IDE·Windows 문서와 5월 14일 mobile preview 발표를 대조해 App-first 시작 경로, JetBrains 지원, Microsoft Store / `winget`, WSL2, iOS/Android 조율 표면, Windows 연결 coming soon을 정리했습니다.",
        ...
        s9_intro: "ChatGPT 웹의 `chatgpt.com/codex`에서는 GitHub 환경을 연결하고 클라우드 코딩 작업을 실행할 수 있습니다. 2026년 5월 14일 OpenAI 발표 기준으로 Codex는 ChatGPT 모바일 앱에서도 iOS/Android preview로 롤아웃 중입니다. 휴대폰은 Codex가 실행 중인 Mac, devbox, managed remote environment에 연결해 active thread, 승인, model 변경, 새 작업 시작, screenshot/terminal output/diff/test result 확인을 이어가는 조율 표면입니다. 파일, credential, permission, local setup은 원래 머신에 남고, Windows Codex App 연결은 coming soon입니다.",
    ...
    en: {
        ...
        changelog_item_3: "Rechecked Quickstart, IDE, Windows docs, and the May 14 mobile preview announcement so the App-first path, JetBrains support, Microsoft Store / `winget`, WSL2, iOS/Android coordination surface, and Windows coming-soon caveat read clearly.",
        ...
        s9_title: "Codex Web/Mobile (ChatGPT)",
        s9_intro: "Use Codex at `chatgpt.com/codex` for cloud-based coding tasks. You can connect a GitHub environment, delegate via `@codex` in a pull request comment, monitor logs, review diffs, and open PRs from the browser. The May 14 OpenAI announcement also adds Codex in the ChatGPT mobile app as an iOS/Android preview. The phone connects to machines where Codex is already running, including a Mac, devbox, or managed remote environment, so you can continue active threads, approve commands, change models, start new work, and review screenshots, terminal output, diffs, and test results. Files, credentials, permissions, and local setup stay on the machine where Codex operates. Windows Codex App connection is coming soon.",
