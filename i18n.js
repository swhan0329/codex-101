...
const translations = {
    ko: {
        ...
        overview_update_5_desc: "`developers.openai.com/codex`, `openai.com/index`, `openai.com/codex` 아래 관련 페이지를 다시 훑어봤습니다. 2026년 5월 13일 `Building a safe, effective sandbox to enable Codex on Windows`는 dedicated sandbox user, firewall 기반 네트워크 차단, command runner를 포함한 `elevated` Windows sandbox 설계 근거로 반영했습니다. 5월 8일 `Running Codex safely at OpenAI`와 5월 7일 `GPT-5.5-Cyber`/Codex Security plugin 발표는 Codex 모델 선택기 변경이 아니라 보안·운영 확장으로 분리했습니다.",
        overview_update_6_title: "Docs MCP·API changelog·설정 기준 재점검",
        overview_update_6_desc: "Docs MCP와 config reference를 다시 확인해 shared CLI/IDE config, AGENTS.md 유도 문구, `review_model`, `model_reasoning_effort`, granular approvals, `approvals_reviewer`, `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, permissions profile, keyring credential storage, forced login/workspace, `sqlite_home`, TUI 설정, OpenTelemetry, apps/plugins/skills 제어 키를 최신 흐름에 맞췄습니다. OpenAI API changelog의 2026년 5월 항목과 API 모델 문서도 별도로 확인해 5월 12일 DALL-E 2/3 snapshot 및 Realtime API Beta 제거, Responses API web search의 `return_token_budget`, Realtime 2/Realtime Translate/Realtime Whisper, OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, Admin API SDK 지원, 그리고 Responses API 전용 `gpt-5-codex` 모델은 Codex 기본 사용법이 아니라 Platform/API 개발자 참고사항으로 분리했습니다.",
        ...
        overview_fact_2_desc: "오늘 기준 `Introducing GPT-5.5`, 최신 `codex/models`, API Models 문서를 함께 보면 `gpt-5.5`가 복잡한 코딩·computer use·knowledge work·research workflow의 최우선 추천 모델이라는 점이 유지됩니다. 다만 오늘 문서 정리에서는 “Codex 안에서 고르는 모델”, “API-key authentication으로 Codex를 쓰는 경우”, “직접 API에서 호출 가능한 모델”을 명확히 분리했습니다. API 자체에서는 `gpt-5.5`와 `gpt-5.5-pro`를 별도 모델로 사용할 수 있고, API 모델 문서의 `gpt-5-codex`는 Responses API 전용 agentic coding 모델로 별도 확인해야 합니다. Codex API-key 사용자는 모델 페이지와 가격표의 차이를 함께 확인해야 합니다. `gpt-5.4`는 rollout 계정이나 비용/속도 기준에서 여전히 강력한 대체 선택지이고, `gpt-5.4-mini`는 빠른 작업과 subagent용 효율 모델로 남아 있습니다.",
        ...
        changelog_item_2: "가격 설명에서 Pro $100/$200 프로모션의 2026년 5월 31일 만료일, Go/iOS 표기, credit 적용 범위, API-key 사용 시 실제 모델 선택기와 직접 API 모델 가격 확인 주의점을 맞췄습니다.",
        changelog_item_3: "Quickstart·IDE·Windows·remote-connections 문서와 5월 14일 mobile preview 발표를 대조해 Mac host 공식 요건과 Windows `remote_control` 운영 메모를 분리했습니다.",
        changelog_item_4: "Cyber Safety 문서를 반영해 일부 보안 작업이 `gpt-5.2`로 reroute될 수 있고 Trusted Access가 필요할 수 있음을 추가했습니다.",
        changelog_item_5: "Docs MCP, best practices, config, OpenAI API changelog, API 모델 문서, Windows sandbox 발표를 다시 확인하고 `gpt-5-codex` 같은 Platform/API 항목은 Codex 기본 사용법과 분리했습니다.",
        changelog_item_6: "Chrome 플러그인으로 X, Threads, LinkedIn을 확인했습니다. 읽힌 소셜 팁 중 검증되지 않은 주장은 넣지 않고, 공식 문서와 맞는 권한 확인, output contract, tool-use rule, completion criteria, OpenAI Developers plugin 활용 메모만 기존 guidance와 연결했습니다.",
        ...
        s9_intro: "ChatGPT 웹의 `chatgpt.com/codex`에서는 GitHub 환경을 연결하고 클라우드 코딩 작업을 실행할 수 있습니다. 2026년 5월 14일 OpenAI 발표 기준으로 Codex는 ChatGPT 모바일 앱에서도 iOS/Android preview로 롤아웃 중입니다. 휴대폰에서는 active thread, 승인, model 변경, 새 작업 시작, screenshot/terminal output/diff/test result 확인을 이어갈 수 있고, 파일·credential·permission·local setup은 원래 host에 남습니다. 다만 현재 `remote-connections` 공식 문서는 mobile setup에 awake/online macOS Codex App host가 필요하고 Windows phone 연결은 coming soon이라고 설명합니다. Windows `remote_control = true` 설정은 해당 빌드에서 기능이 보일 때 쓰는 운영 메모로 분리해 읽으세요.",
        ...
        s13_refs: '출처: <a href="https://developers.openai.com/codex/learn/best-practices/" target="_blank">Codex Best practices</a> · <a href="https://developers.openai.com/codex/config-reference/" target="_blank">config reference</a> · <a href="https://developers.openai.com/codex/concepts/sandboxing/" target="_blank">sandboxing</a> · <a href="https://openai.com/index/building-codex-windows-sandbox/" target="_blank">Windows sandbox engineering</a> · <a href="https://openai.com/index/running-codex-safely/" target="_blank">Running Codex safely</a> · <a href="https://openai.com/index/openai-on-aws/" target="_blank">OpenAI on AWS</a>',
        // Section 13
        ...
        s18_field_credit: "이번 실행에서는 Chrome 연결이 성공했고 X, Threads, LinkedIn 모두에서 읽을 수 있는 검색 결과 일부를 확인했습니다. 검증되지 않은 주장은 추가하지 않았고, 공식 문서와 맞는 권한 확인, 출력 계약, 도구 사용 규칙, 완료 기준, OpenAI Developers plugin 활용 메모만 기존 prompting guidance와 연결했습니다.",
    ...
    en: {
        ...
        overview_update_1_desc: "The May 20 run started by re-checking `codex/models` first, confirming that `gpt-5.5` is still the top recommendation, that `gpt-5.4` remains the fallback path, and clarifying how `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark` split fast local/subagent, Cloud/code-review, and Pro research-preview roles. The key API-key nuance remains: `codex/models` marks GPT-5.5 API Access as true, while the `codex/pricing` API Key table still lists it as not available.",
        overview_update_2_title: "Pricing and credits refreshed",
        overview_update_2_desc: "The guide keeps the `GPT-5.5` usage tables, `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Plus web/CLI/IDE/iOS access, Business/Enterprise credits language, and the current Plus / Pro 5x / Pro 20x / Business / API Key pricing layout aligned. The May 20 check also records that Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 gets a usage boost through the same date. This run also makes the split clearer: API-key users should verify the actual Codex model picker and standard API pricing, while direct OpenAI API model access is documented separately.",
        overview_update_3_title: "Quickstart, IDE, and mobile path tightened",
        overview_update_3_desc: "The top path still keeps App-first onboarding separate from IDE/CLI + Docs MCP professional workflows in Quickstart, but now also spells out API-key sign-in limits and the Windows/WSL path more clearly. The May 14 OpenAI announcement adds Codex in the ChatGPT mobile app as an iOS/Android preview for continuing threads, approvals, model changes, new work, screenshots, terminal output, diffs, and test results. The current `remote-connections` doc is stricter: mobile setup currently requires a macOS Codex App host and says Windows phone connection is coming soon, so the guide separates that from the user-provided Windows `remote_control` operating memo.",
        ...
        overview_update_5_desc: "I re-read `developers.openai.com/codex`, `openai.com/index`, and `openai.com/codex` together. The May 13 `Building a safe, effective sandbox to enable Codex on Windows` post is now reflected as the engineering basis for the preferred `elevated` Windows sandbox, including dedicated sandbox users, firewall-backed network blocking, and a command runner. The May 8 `Running Codex safely at OpenAI` and May 7 `GPT-5.5-Cyber` / Codex Security launches remain security-operations context, not Codex model-picker changes.",
        overview_update_6_title: "Docs MCP, API changelog, and config rechecked",
        overview_update_6_desc: "Docs MCP and the config reference were rechecked around shared CLI/IDE setup, the AGENTS.md steering snippet, `review_model`, `model_reasoning_effort`, granular approvals, `approvals_reviewer`, `web_search`, `tools.web_search`, `tools.view_image`, `tool_suggest`, permissions profiles, keyring credential storage, forced login/workspace, `sqlite_home`, TUI settings, telemetry, and app/plugin/skill controls. The May 2026 OpenAI API changelog and API model docs were also checked separately: the May 12 removal of DALL-E 2/3 snapshots and the Realtime API Beta, Responses API web search `return_token_budget`, Realtime 2 / Realtime Translate / Realtime Whisper, the OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, Admin API SDK support, and the Responses API-only `gpt-5-codex` model are Platform/API developer updates, not Codex model-picker changes.",
        ...
        overview_fact_2_desc: "The latest model story is now split more clearly. `gpt-5.5` is still the top recommendation for complex coding, computer use, knowledge work, and research workflows in Codex. In this refresh, the guide separates the Codex model picker, API-key authentication for Codex, and direct API model calls. The API itself exposes `gpt-5.5` and `gpt-5.5-pro` as separate models, and the API model docs list `gpt-5-codex` as a Responses API-only agentic coding model. API-key Codex users should compare the models page, pricing table, and actual product picker. `gpt-5.4` still matters for rollout, cost, or latency tradeoffs, while `gpt-5.4-mini` remains the efficient choice for lighter work and subagents.",
        ...
        changelog_item_2: "Realigned the Pro $100/$200 promo end date of May 31, 2026, Go/iOS plan wording, credit scope, and the warning that API-key users should verify the actual picker plus standard API model pricing.",
        changelog_item_3: "Rechecked Quickstart, IDE, Windows, remote-connections, and the May 14 mobile preview announcement; official Mac-host requirements are now separated from the Windows `remote_control` operating memo.",
        ...
        changelog_item_5: "Rechecked Docs MCP, best practices, config, the OpenAI API changelog, API model docs, and the Windows sandbox post; Platform/API items like `gpt-5-codex` stay separate from core Codex usage.",
        changelog_item_6: "The Chrome plugin checked X, Threads, and LinkedIn. Unverified social claims were not added; official-doc-aligned notes on permissions, output contracts, tool rules, done criteria, and the OpenAI Developers plugin were folded into existing guidance.",
        ...
        s9_title: "Codex Web/Mobile (ChatGPT)",
        s9_intro: "Use Codex at `chatgpt.com/codex` for cloud-based coding tasks. You can connect a GitHub environment, delegate via `@codex` in a pull request comment, monitor logs, review diffs, and open PRs from the browser. The May 14 OpenAI announcement also adds Codex in the ChatGPT mobile app as an iOS/Android preview for continuing active threads, approving commands, changing models, starting work, and reviewing screenshots, terminal output, diffs, and test results. Files, credentials, permissions, and local setup stay on the host. The current `remote-connections` doc says mobile setup requires an awake, online macOS Codex App host and that Windows phone connection is coming soon. Treat `remote_control = true` under `[features]` as a separate Windows operating note for builds where that capability appears.",
        ...
        s18_field_credit: "Chrome connected successfully in this run, and readable X, Threads, and LinkedIn search results were available. I did not add unverified claims; I only tied official-doc-aligned notes on permissions, output contracts, tool-use rules, done criteria, and the OpenAI Developers plugin back to existing guidance.",
