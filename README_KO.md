...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 1일</strong><br/>
...
- Quickstart, Pricing, Best practices, Config, Windows/Windows App/IDE/App features/App Automations/Cloud, Remote connections, Integrations, MCP, Hooks, Plugins, Skills, Sites, OpenAI API changelog와 Platform/API 문서를 다시 확인했습니다. remote-control, Sites, Windows App 활성화 절차는 바뀌지 않았습니다.
- `openai.com/index`에서 2026년 6월 30일 게시된 [How people are using ChatGPT](https://openai.com/index/global-ai-and-chatgpt-adoption/), [GeneBench-Pro](https://openai.com/index/genebench-pro/), [Core dump debugging and epidemiology](https://openai.com/index/core-dump-debugging-epidemiology/)를 추가 확인했습니다. 각각 global adoption research, 생명과학 benchmark, data infrastructure debugging 사례로만 분리했고, Codex 모델, 플랜, preview/GA, feature flag, activation, credential, permission 변화로 승격하지 않았습니다.
...
- 공식 use-case catalog는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 저장된 social research note를 baseline으로 다시 확인했습니다. Chrome에서 X는 빈 body, Threads는 검색 shell만 반환했고 LinkedIn은 blank tab으로 떨어져, 새로 공식 문서로 대조 가능한 social-only Codex 팁은 추가하지 않았습니다.
...
- 2026년 7월 1일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다. `slack-coding-tasks`와 `github-code-reviews`는 공식 페이지가 starter prompt를 open-in-Codex 버튼이 아니라 copy/export block에 싣는 예외로 계속 분리해 확인합니다.
