...
<p align="center">
  <strong>최종 업데이트: 2026년 4월 6일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-04-06)

- `codex/models`를 다시 대조한 결과, 추천 모델은 오늘도 `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.3-codex-spark`의 4개 축입니다. 지금 문서에서 특히 유지해야 할 핵심은 `gpt-5.4`와 `gpt-5.4-mini`가 로컬 기본 선택지이고, `gpt-5.3-codex`만 Codex Cloud가 켜진 추천 축이라는 점입니다.
...
- 가격/접근 범위는 이번에도 Quickstart, Pricing, 그리고 OpenAI 가격 발표를 함께 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고, Pricing은 개인과 팀의 읽는 법을 더 분명히 나눕니다. 개인은 Free/Go/Plus/Pro 한도표를, 팀은 Business·Enterprise의 usage-based seat, 크레딧 프로모션, 토큰 기반 과금 전환 여부를 함께 봐야 최신 해석과 맞습니다.
- 설치 흐름은 더 분명해졌습니다. Quickstart는 앱을 가장 쉬운 시작 경로로 두고, IDE는 VS Code, Cursor, Windsurf, JetBrains를 함께 다루며, API 키 로그인도 가능하지만 cloud threads 같은 기능은 제한될 수 있다고 안내합니다.
- Windows 문서는 이제 일반 Windows 가이드와 전용 Windows App 문서가 역할을 나눕니다. 기본 권장은 네이티브 앱이고, Microsoft Store나 `winget`로 설치할 수 있으며, `elevated` 샌드박스를 우선 쓰고 `unelevated`는 대안으로 봅니다. 전용 데스크톱 격리가 기본이고, IDE의 Windows agent mode는 여전히 WSL 워크스페이스를 가장 안정적인 경로로 보는 편이 맞습니다.
- 현재 app/features 문서를 보면 Codex App은 단순한 데스크톱 셸이 아니라 worktrees, Automations, Git 도구, 내장 terminal, voice dictation, pop-out window, IDE sync, image input, notifications, 절전 방지까지 묶은 작업 허브로 설명됩니다.
...
- `config.toml` 최신화 포인트도 더 넓어졌습니다. 현재 `config-basic` 문서는 precedence, `web_search = "cached"` 기본값, `personality`, `shell_environment_policy`, feature flag, `windows.sandbox`, `windows.sandbox_private_desktop`, granular approval policy까지 함께 다룹니다.
