...
<p align="center">
  <strong>최종 업데이트: 2026년 8월 18일</strong><br/>
...
- What's new와 Codex changelog를 다시 확인했습니다. CLI 0.147.0은 MCP server startup 전 cached MCP tool 노출, model-owned token budget defaults, Enterprise automation account plan support, current-turn permission/profile 처리 강화, MCP 2026 client/discovery support, Agent Plugin runtime boundary 작업, remote plugin search, portable Agent Plugins, app-server/thread/history 개선을 포함합니다. 또한 legacy `codex exec --full-auto` flag가 제거됐으므로 예시는 `--sandbox workspace-write`와 명시적 approval flag 조합으로 고쳤습니다. 이전 0.146.0의 named chat, pinned thread, temporary fork, standalone web search, executor-provided skills, Agent Plugin manifest, workspace plugin publishing, app-server/WebSocket 개선도 계속 반영되어 있습니다.
- 공식 use-case catalog는 계속 101개입니다. 공식 Starter prompt 77개의 줄바꿈 또는 blank-line 구조 변경을 `promptEn`과 `promptKo`에 맞췄고, 공식 상세 101개 `starterPrompt.body`와 local `official.promptEn` mismatch는 0입니다. 첫 카드의 canonical slug는 `daily-work-brief`이며, 기존 `proactive-teammate` hash는 alias로 유지했습니다.
...
- 공식 use-case catalog는 collection page를 제외한 local 카드 101개 기준으로 다시 감사했습니다. 공식 개별 페이지 101개, local 카드 101개, `promptEn` 누락 0, `promptKo` 누락 0, 공식 Starter prompt mismatch 0, fetch error 0, line/token 구조 회귀 0입니다. `use-cases-data.js` scrape date는 2026-08-18로 맞추고 공식 prompt 줄구조를 보존했습니다.
