...
<p align="center">
  <strong>최종 업데이트: 2026년 5월 31일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-05-31)

- 2026년 5월 31일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex`는 cloud/code review 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축, `gpt-5.2`는 대표 대안 모델로 정리하는 편이 맞습니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주며, `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `API Key` 기준의 사용량 표를 유지합니다. Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. Plus 설명에는 이제 web, CLI, IDE extension, iOS 접근도 함께 표시됩니다. 2026년 5월 31일 확인 기준으로 Pro $100은 2026년 5월 31일까지 2x 추가 사용량을 받습니다. Pro $200은 20x Plus가 상시 포함되고, 5시간 Codex 한도는 2026년 5월 31일까지 25x Plus 수준으로 유지됩니다.
...
- `developers.openai.com/codex`, 관련 OpenAI 발표/검색 표면, API 모델 문서, Help Center 릴리스 노트, OpenAI API changelog도 폭넓게 다시 훑어봤지만, 오늘 기준으로 Codex 모델 추천 순서는 바뀌지 않았습니다. 이번에는 2026년 5월 29일 release note의 Windows Computer Use, Windows host remote control, responsiveness/browser stability, Codex Profiles 설명을 명시적으로 반영했고, 2026년 5월 21일의 Appshots, Goal mode GA, locked computer use, in-app browser annotation, Codex admin analytics, plugin sharing 기본값도 유지했습니다. 2026년 5월 27일 OpenAI engineering 글 [Building self-improving tax agents with Codex](https://openai.com/index/building-self-improving-tax-agents-with-codex/)는 새 설정 토글이 아니라 고급 운영 패턴으로 유지했습니다. Chrome 소셜 확인에서는 X 본문은 읽히지 않았고 Threads에는 Appshots/oh-my-codex 관련 커뮤니티 글, LinkedIn에는 Profile/token usage 관련 글이 보였지만, 공식 문서로 재확인된 주장만 추가했습니다.
...
2026년 5월 31일 실행 참고: 공식 문서와 릴리스 노트는 이제 Windows host를 ChatGPT 모바일 또는 Mac에서 제어할 수 있다고 설명하지만, Windows Codex App은 아직 다른 컴퓨터를 제어하는 클라이언트가 아니라고 제한합니다. Windows Computer Use는 eligible user에게 새로 문서화됐지만 출시 시점 지역 제한이 있으므로 전 사용자 기능처럼 설명하지 않았습니다.
