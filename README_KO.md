...
<p align="center">
  <strong>최종 업데이트: 2026년 5월 23일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-05-23)

- 2026년 5월 23일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex`는 cloud/code review 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축, `gpt-5.2`는 대표 대안 모델로 정리하는 편이 맞습니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주며, `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `API Key` 기준의 사용량 표를 유지합니다. Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. Plus 설명에는 이제 web, CLI, IDE extension, iOS 접근도 함께 표시됩니다. 2026년 5월 23일 확인 기준으로 Pro $100은 2026년 5월 31일까지 2x 추가 사용량을 받습니다. Pro $200은 20x Plus가 상시 포함되고, 5시간 Codex 한도는 2026년 5월 31일까지 25x Plus 수준으로 유지됩니다.
...
2026년 5월 23일 실행 참고: Chrome 연결은 성공했고 X, Threads, LinkedIn 모두에서 읽을 수 있는 검색 결과 일부를 확인했습니다. X에서는 locked Mac control, Appshots, Goal mode 관련 공식/커뮤니티 글이 보였고, Threads와 LinkedIn에서는 Windows `remote_control` flag 경로를 반복하는 community post가 있었습니다. 공개 공식 문서는 아직 Windows phone 연결을 coming soon으로 설명하므로 일반 availability 문장이 아니라 운영 메모로만 유지했습니다.
