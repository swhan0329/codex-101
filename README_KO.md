...
<p align="center">
  <strong>최종 업데이트: 2026년 6월 4일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-06-04)

- 2026년 6월 4일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex`는 cloud/code review 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축, `gpt-5.2`는 대표 대안 모델로 정리하는 편이 맞습니다. 임의 provider로 Codex를 연결할 때 Chat Completions API 지원은 deprecated이며 future release에서 제거될 예정이므로, 새 설정은 Responses API 지원 provider를 기준으로 잡는 편이 안전합니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주며, `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `API Key` 기준의 사용량 표를 유지합니다. Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. 이전 Pro 프로모션 문구는 현재 pricing 문서에서 사라졌고, Pro는 Plus 대비 5x 또는 20x higher rate limits로 설명됩니다. Plus 설명에는 web, CLI, IDE extension, iOS 접근도 함께 표시됩니다. 가격표의 feature availability에는 Sites가 Business/Enterprise에서 available, Plus/Pro/API Key에서는 unavailable로 표시되고, Sites FAQ는 preview 기간 동안 무료라고 설명합니다. ChatGPT for Excel 같은 agentic feature도 유효 시점 이후 Codex 사용 한도를 공유할 수 있습니다.
...
- `developers.openai.com/codex`, 관련 OpenAI 발표/검색 표면, API 모델 문서, Help Center 릴리스 노트, OpenAI API changelog도 폭넓게 다시 훑어봤지만, 오늘 기준으로 Codex 모델 추천 순서는 바뀌지 않았습니다. 대신 2026년 6월 2일 [Codex for every role, tool, and workflow](https://openai.com/index/codex-for-every-role-tool-workflow/) 발표와 공식 Sites 문서를 workflow 확장으로 유지하고, 6월 2일 knowledge-work 보고서와 5월 13일 macOS certificate update 공지를 추가했습니다. Chrome 소셜 확인에서는 X에 usage reset류 비공식 claim이 보였고, LinkedIn은 공식 발표 반복과 Wix Headless 파트너 사례를 보여줬으며, Threads는 로드 제한으로 읽지 못했습니다. 검증되지 않은 claim은 넣지 않았습니다.
...
2026년 6월 4일 실행 참고: Sites는 일반 preview 배포와 분리해서 읽어야 합니다. Business/Enterprise preview, OpenAI-hosted production URL, RBAC/admin 설정, 접근 권한, saved version 검토, `.openai/hosting.json`의 project/storage binding, Sites panel의 runtime secret 처리가 함께 걸린 Codex App workflow입니다.
