...
<p align="center">
  <strong>최종 업데이트: 2026년 8월 3일</strong><br/>
...
- 최신 Codex changelog를 다시 확인했고 CLI 0.145.0를 추가했습니다. paginated thread history, 확장된 `/import`, Bedrock endpoint/auth, audio inputs/tool outputs, realtime V3, multi-agent V2 안정화, inline visualization link, Windows/MCP reliability 개선을 운영 메모로 반영했습니다.
...
- openai.com/index와 OpenAI Developers/API 문서도 다시 대조했습니다. 오늘은 Codex 제품 추천 모델을 바꿀 항목은 없었고, GPT-5.6 가격/성능 발표와 ARC-AGI-3 harness 글은 Codex 제품 설정이 아니라 API/평가 운영 맥락으로 분리했습니다. NTT DATA Group Codex 사례는 9,000명 규모 Enterprise rollout과 incident analysis 자동화 사례로만 두고, 일반 사용자 기능이나 모델 순서 변경으로 쓰지 않았습니다.
- Chrome으로 X, LinkedIn, Threads의 사람별 공개 practitioner signal을 다시 확인했습니다. Section 20에는 VB의 중간 확인 요청 팁을 추가했습니다. `default_mode_request_user_input`은 공식 config schema에 feature key로 보이지만, 동작 설명은 practitioner context로 분리했고 팀 기본값에 넣기 전 현재 앱 동작 확인이 필요하다고 표시했습니다.
...
- 2026년 8월 3일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 101개이고, 공식 Starter prompt 101개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. local 누락 0, official URL 누락 0, `promptEn` 누락 0, `promptKo` 누락 0, line/token 구조 mismatch 0이며, 오늘 새 Starter prompt 변경은 없었습니다.
