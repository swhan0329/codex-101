...
<p align="center">
  <strong>최종 업데이트: 2026년 6월 1일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-06-01)

- 2026년 6월 1일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex`는 cloud/code review 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축, `gpt-5.2`는 대표 대안 모델로 정리하는 편이 맞습니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주며, `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `API Key` 기준의 사용량 표를 유지합니다. Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. Plus 설명에는 이제 web, CLI, IDE extension, iOS 접근도 함께 표시됩니다. 2026년 6월 1일 확인 기준으로 공식 문서와 Help Center는 Pro $100 2x 및 Pro $200 25x 5시간 한도 부스트를 2026년 5월 31일까지로 설명합니다. 종료일이 지난 프로모션이므로 실제 남은 한도는 Codex usage dashboard와 플랜 배너에서 다시 확인해야 합니다.
...
- `developers.openai.com/codex`, 관련 OpenAI 발표/검색 표면, API 모델 문서, Help Center 릴리스 노트, OpenAI API changelog도 폭넓게 다시 훑어봤지만, 오늘 기준으로 Codex 모델 추천 순서는 바뀌지 않았습니다. 이번에는 2026년 5월 29일 release note의 Windows Computer Use, Windows host remote control, responsiveness/browser stability, Codex Profiles 설명을 유지했고, 2026년 5월 21일의 Appshots, Goal mode GA, locked computer use, in-app browser annotation, Codex admin analytics, plugin sharing 기본값도 유지했습니다. 2026년 5월 29일 OpenAI 고객 사례 [How Braintrust turns customer requests into code with Codex](https://openai.com/index/braintrust/)와 공식 자료 [How OpenAI uses Codex](https://openai.com/business/guides-and-resources/how-openai-uses-codex/)는 고객 요청을 preview branch로 빠르게 검증하고, Ask Mode/이슈형 프롬프트/환경 개선/AGENTS.md/Best-of-N을 쓰는 실무 패턴으로 반영했습니다. 2026년 5월 27일 OpenAI engineering 글 [Building self-improving tax agents with Codex](https://openai.com/index/building-self-improving-tax-agents-with-codex/)는 새 설정 토글이 아니라 고급 운영 패턴으로 유지했습니다. Chrome 소셜 확인에서는 X가 읽혔지만 사용량 리셋 등 공식 문서로 검증되지 않은 claim이 섞여 있어 추가하지 않았고, Threads/LinkedIn도 공식 문서로 재확인된 주장만 반영했습니다.
...
2026년 6월 1일 실행 참고: 공식 문서와 릴리스 노트는 이제 Windows host를 ChatGPT 모바일 또는 Mac에서 제어할 수 있다고 설명하지만, Windows Codex App은 아직 다른 컴퓨터를 제어하는 클라이언트가 아니라고 제한합니다. Windows Computer Use는 eligible user에게 새로 문서화됐지만 출시 시점 지역 제한이 있으므로 전 사용자 기능처럼 설명하지 않았습니다.
