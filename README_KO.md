...
<p align="center">
  <strong>최종 업데이트: 2026년 6월 30일</strong><br/>
...
- `openai.com/index`에서 2026년 6월 29일 [Mapping AI job transition pathways across the EU](https://openai.com/index/mapping-ai-jobs-transition-eu/)를 확인했습니다. 이는 workforce transition/research 맥락이며 Codex 모델, 플랜, preview/GA, feature flag, activation, credential, permission 변화가 아니므로 제품 안내로 승격하지 않았습니다.
...
- 저장된 social research note를 baseline으로 다시 확인했습니다. Chrome에서 X/Threads/LinkedIn을 열 수는 있었지만 X/Threads는 현재 글 본문을 신뢰할 만큼 노출하지 않았고, LinkedIn은 기존 Codex Remote/agentic-work 맥락 중심이어서 새로 공식 문서로 대조 가능한 social-only Codex 팁은 추가하지 않았습니다.
...
- 2026년 6월 30일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다. `slack-coding-tasks`와 `github-code-reviews`는 공식 페이지가 starter prompt를 open-in-Codex 버튼이 아니라 copy/export block에 싣는 예외로 계속 분리해 확인합니다.
