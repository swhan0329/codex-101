...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 29일</strong><br/>
...
- OpenAI API latest-model/changelog는 별도 Platform/API lane으로 반영했습니다. `gpt-5.6` alias는 `gpt-5.6-sol`로 라우팅되고, Programmatic Tool Calling, Multi-agent beta, explicit prompt caching, persisted reasoning, max reasoning effort, Pro mode, original/auto image detail preservation이 추가됐습니다. API hard spend limits는 monthly cap에 닿으면 429를 반환할 수 있으므로 Codex 제품 설정이 아니라 Platform/API 운영 기준으로 분리했습니다.
- 공식 use-case catalog는 collection page를 제외한 local 카드 99개 기준으로 다시 감사했습니다. 공식 개별 페이지 99개, local 카드 99개, `promptEn` 누락 0, `promptKo` 누락 0, 공식 Starter prompt mismatch 0, line/token 구조 회귀 0입니다. 오늘 바뀐 Starter prompt 5개는 dashboard, inbox zero, slide deck, datasets/reports, internal apps 카드에 반영했습니다.
...
- 2026년 7월 29일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 99개로 유지됐고, 변경된 Starter prompt 5개를 반영한 뒤 공식 Starter prompt 99개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
