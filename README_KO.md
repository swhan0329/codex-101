...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 5일</strong><br/>
...
- `openai.com/index`에서는 [Simplex rethinks software development with Codex](https://openai.com/index/simplex/)와 [Dreaming: Better memory for a more helpful ChatGPT](https://openai.com/index/chatgpt-memory-dreaming/)를 추가 확인했습니다. Simplex는 ChatGPT Enterprise와 Codex를 쓰는 enterprise adoption 및 AI-first delivery 사례이고, Dreaming은 ChatGPT memory 제품 업데이트입니다. 둘 다 Codex 모델, 플랜, preview/GA, feature flag, activation, credential, permission 변화로 승격하지 않았습니다.
...
- 공식 use-case catalog는 collection page를 제외한 실제 카드 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 저장된 social research note를 baseline으로 다시 확인했습니다. 이번 세션에는 Chrome 전용 제어 도구가 노출되지 않았고, Playwright 번들 Chromium은 설치되어 있지 않았으며, system Chrome fallback에서는 X/LinkedIn이 로그인으로 redirect되고 Threads는 noisy public post만 노출했습니다. 검증되지 않은 소셜 claim은 공개 가이드에 추가하지 않았습니다.
...
- 2026년 7월 5일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
