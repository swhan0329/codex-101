...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 30일</strong><br/>
...
- Quickstart, Pricing, Speed/Fast mode, Best practices, Config, Windows/Windows App/IDE/App features/App Automations/Cloud, Remote connections, Integrations, MCP, Hooks, Plugins, Skills, Sites, OpenAI API latest-model/changelog와 Platform/API 문서를 다시 확인했습니다. Fast mode는 이제 GPT-5.6, GPT-5.5, GPT-5.4를 지원하고, GPT-5.6/GPT-5.5는 Standard 대비 2.5배, GPT-5.4는 2배 ChatGPT credit을 씁니다. API-key workflow는 Fast credit이 아니라 API token pricing을 따르므로 분리했습니다.
...
- OpenAI API latest-model/changelog는 별도 Platform/API lane으로 반영했습니다. `gpt-5.6` alias는 `gpt-5.6-sol`로 라우팅되고, Programmatic Tool Calling, Multi-agent beta, explicit prompt caching, persisted reasoning, max reasoning effort, Pro mode, original/auto image detail preservation이 추가됐습니다. API hard spend limits와 2026년 7월 29일 Terraform provider는 Platform/API 운영 기준으로 분리했고, 같은 날 GPT-5.6 efficiency engineering 글은 Codex/ChatGPT Work의 deferred discovery, tool output cap, prompt-cache prefix 안정성 맥락으로만 반영했습니다.
- 공식 use-case catalog는 collection page를 제외한 local 카드 101개 기준으로 다시 감사했습니다. 공식 개별 페이지 101개, local 카드 101개, `promptEn` 누락 0, `promptKo` 누락 0, 공식 Starter prompt mismatch 0, line/token 구조 회귀 0입니다. 신규 카드 `project-teammate`, `track-bills-subscriptions-and-spending`를 추가했고, 오늘 바뀐 Starter prompt 3개는 daily work brief, feedback synthesis, computer use 카드에 반영했습니다.
...
- openai.com/index와 OpenAI Developers/API 문서도 다시 대조했습니다. 오늘은 모델 추천 변경으로 넓게 바꿔야 할 항목은 없었고, 2026년 7월 28일 scientific computing field report는 검증/stewardship 맥락으로만, 2026년 7월 22일 NTT DATA 사례는 Enterprise CoE/보안/adoption 맥락으로만 반영했습니다. Platform/API 메모는 Codex 제품 안내와 계속 분리했습니다.
- Chrome으로 X, LinkedIn, Threads의 사람별 공개 practitioner signal을 다시 확인했습니다. 새로 승격할 OpenAI/Codex 담당자 개인 글은 없었고, Section 20의 기존 practitioner card는 원글 또는 공식 corroborating docs에 연결된 상태로 유지했습니다.
...
- 2026년 7월 30일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 101개로 늘었고, 신규 2개와 변경된 Starter prompt 3개를 반영한 뒤 공식 Starter prompt 101개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
