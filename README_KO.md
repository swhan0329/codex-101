...
<p align="center">
  <strong>최종 업데이트: 2026년 8월 1일</strong><br/>
...
- openai.com/index와 OpenAI Developers/API 문서도 다시 대조했습니다. 오늘은 Codex 제품 추천 모델을 바꿀 항목은 없었고, GPT-5.6 가격/성능 발표와 ARC-AGI-3 harness 글은 Codex 제품 설정이 아니라 API/평가 운영 맥락으로 분리했습니다. Platform/API 메모는 Codex 제품 안내와 계속 분리했습니다.
- Chrome으로 X, LinkedIn, Threads의 사람별 공개 practitioner signal을 다시 확인했습니다. 새로 승격할 OpenAI/Codex 담당자 개인 글은 없었고, Threads에서 보인 GPT-5.6 Luna/Terra 가격 인하 신호는 공식 발표로 이미 대조되어 제품 사실은 공식 발표 기준으로만 반영했습니다. Section 20의 기존 practitioner card는 원글 또는 공식 corroborating docs에 연결된 상태로 유지했습니다.
...
- 2026년 8월 1일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 101개이고, 공식 Starter prompt 101개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. local 누락 0, official URL 누락 0, `promptEn` 누락 0, `promptKo` 누락 0, line/token 구조 mismatch 0이며, 오늘 새 Starter prompt 변경은 없었습니다.
