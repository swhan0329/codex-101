...
- What's new와 Codex changelog를 다시 확인했습니다. CLI 0.149.0은 interactive `codex agents` task dashboard, TUI의 `/cd`·`/pwd`·`/cwd`, 기존 session에 메시지를 보내는 `codex queue`를 추가했습니다. `codex doctor`는 endpoint protection, network/proxy failure, desktop app state, update connectivity까지 진단하며, resume/fork한 thread는 active permission profile도 복원합니다. 0.148.0의 export, fork, archive/restore, Hooks 변경은 계속 사용할 수 있습니다.
