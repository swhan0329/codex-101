...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 28일</strong><br/>
...
- 공식 use-case catalog는 collection page 13개를 제외한 local 카드 99개 기준으로 다시 감사했습니다. 공식 개별 페이지 99개, local 카드 99개, `promptEn` 누락 0, `promptKo` 누락 0, 공식 Starter prompt mismatch 0, line/token 구조 회귀 0입니다. 일부 상세 페이지는 deep-link prompt prop 대신 visible prompt block을 쓰므로 fallback extractor로 다시 확인했습니다.
- 2026년 7월 20일 OpenAI Developers의 Custom Code Review rules for Codex 글을 섹션 16에 반영했습니다. GitHub Code Review는 가까운 `AGENTS.md`의 `## Code Review Rules`를 읽고, 팀이 반복해서 설명하는 호환성·데이터 경계·안전한 대안을 짧은 규칙으로 남기면 리뷰에서 해당 근거를 인용할 수 있습니다. 포맷팅과 lint처럼 결정적인 검사는 CI에 두고, 규칙은 실제로 놓치면 큰 repository-specific invariant에 집중해야 합니다.
- openai.com/index 발표도 다시 대조했습니다. 2026년 6월 25일 agents-at-work 연구 글은 Codex adoption/economic context로만 유지하고, 모바일 원격 제어 발표는 현재 `remote-connections` 운영 문서와 계속 분리해 설명합니다. Health in ChatGPT는 Codex 기능으로 다루지 않습니다.
...
- 2026년 7월 28일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 99개로 유지됐고, 공식 Starter prompt 99개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
