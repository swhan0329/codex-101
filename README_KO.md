...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 16일</strong><br/>
...
- 최신 Codex changelog를 반영했습니다. iOS 1.2026.188은 inline visualization과 mobile task flow를 개선했고, CLI 0.144.0은 reset credit 상세 표시, `writes` app approval mode, MCP auth elicitation 기본화, Ultra 사용량 경고, Windows sandbox fix, system proxy 안정성을 추가했습니다. CLI 0.144.1은 installer/code-mode reliability를 고쳤고, CLI 0.144.2는 prompting regression rollback 후 Guardian auto-review 동작을 이전 방식으로 복원했습니다. CLI 0.144.3/0.144.4는 version-only 또는 user-facing change가 없는 release입니다.
...
- 새로 공개 가이드에 승격할 공식 근거가 있는 person-linked practitioner tip은 없어서 Section 20의 cards는 기존 공식 근거와 원문 링크 기준으로 유지했습니다.
...
- 2026년 7월 16일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 99개로 유지됐고, 공식 Starter prompt 99개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
