...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 8일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-07-08)

- `codex/models`를 가장 먼저 다시 확인했고, 추천 순서는 `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex-spark`로 유지됐습니다. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 인증 Codex 기준 deprecated로 정리하고, 더 이상 대체 추천 모델처럼 설명하지 않았습니다.
- Quickstart, Pricing, Speed/Fast mode, Best practices, Config, Windows/Windows App/IDE/App features/App Automations/Cloud, Remote connections, Integrations, MCP, Hooks, Plugins, Skills, Sites, OpenAI API changelog와 Platform/API 문서를 다시 확인했습니다. remote-control, Sites, Windows App 활성화 절차는 바뀌지 않았습니다.
- Codex changelog의 2026-07-01 `Codex CLI 0.142.5`를 새로 반영했습니다. Responses WebSocket request payload가 trace log에 남지 않도록 막은 bug fix라서, 새 기능이나 모델 순서 변화가 아니라 CLI 보안/로그 hygiene 메모로 분리했습니다.
- Pricing은 `Free $0`, `Go $8`, `Plus $20`, `Pro from $100`, Business, Enterprise & Edu, API Key 구조를 유지합니다. Fast mode는 ChatGPT 로그인으로 Codex app/CLI/IDE extension을 쓸 때 GPT-5.5와 GPT-5.4를 1.5배 빠르게 돌리는 설정이고, credit은 각각 standard 대비 2.5배/2배 더 빨리 소모합니다. API-key Codex는 Fast mode credit이 아니라 standard API pricing을 따릅니다.
- `developers.openai.com/blog`에서는 [Mastering Codex Remote for engineering](https://developers.openai.com/blog/mastering-codex-remote-for-engineering)과 [Making private MCP servers reachable without making them public](https://developers.openai.com/blog/connect-private-mcp-servers-to-openai-products)을 다시 확인했습니다. 전자는 Remote connections 문서의 phone-as-control-center 운영 패턴을 보강하고, 후자는 Secure MCP Tunnel/Platform API 보안 연결 맥락으로만 둡니다. 둘 다 Codex 모델 순서, 일반 사용자 활성화 절차, credential 이동 변화는 아닙니다.
- `developers.openai.com/blog`와 `openai.com/index`를 다시 확인했습니다. 공개 OpenAI news index의 최신 항목은 여전히 2026년 6월 30일 글들이어서 오늘 새 Codex 제품 발표는 추가하지 않았습니다. 기존 Codex Remote, private MCP server/Secure MCP Tunnel, Simplex, ChatGPT Dreaming 메모는 각각 운영, Platform/API, enterprise adoption, ChatGPT memory 맥락으로 분리해 두었습니다.
- 2026년 6월 26일 [Previewing GPT-5.6 Sol](https://openai.com/index/previewing-gpt-5-6-sol/)은 select trusted partners/organizations 대상 preview이며 broader ChatGPT, Codex, API availability는 coming soon으로 설명되므로, 일반 Codex 모델 선택기 추천 순서나 일반 사용자 활성화 절차로 승격하지 않았습니다.
- 2026년 6월 28일 [HP Frontier partnership](https://openai.com/index/hp-frontier-partnership/)는 HP가 thousands of employees across roles에 ChatGPT Enterprise와 Codex를 배포한다는 enterprise adoption 사례로만 반영했습니다. 새 Codex 모델, 소비자 플랜, preview/GA 상태, 설정 키, feature flag, credential/permission 변화는 아닙니다.
- [OpenAI frontier models and Codex are now available on AWS](https://openai.com/index/openai-frontier-models-and-codex-are-now-available-on-aws/)는 Codex on Amazon Bedrock의 enterprise/provider 배포 맥락입니다. Commercial/GovCloud AWS 운영 모델, AWS-native security/governance/procurement/billing을 쓰는 경로이지 ChatGPT 플랜 한도, Codex 모델 선택 순서, local app 설정 키 변화가 아닙니다.
- [Introducing workspace agents in ChatGPT](https://openai.com/index/introducing-workspace-agents-in-chatgpt/)는 Codex-powered cloud agents를 ChatGPT Business, Enterprise, Edu, Teachers plans의 research preview로 제공하는 별도 ChatGPT 팀 기능입니다. Agents sidebar에서 만들고 ChatGPT/Slack에서 쓰며, 조직 permission, connected tools, approval, analytics, Compliance API, RBAC로 관리합니다. Codex app 지원은 “coming soon”으로 설명되어 있어 현재 Codex App automations나 local thread automation과 섞지 않았습니다.
- 공식 use-case catalog는 collection page를 제외한 실제 카드 60개로 유지됐고, `use-your-computer-with-codex`, `deploy-app-or-website`, `qa-your-app-with-computer-use`, `complete-tasks-from-messages`, `follow-goals`, `chatgpt-apps`의 Starter prompt drift를 반영했습니다. 특히 `follow-goals`는 짧은 `/goal Complete [objective]...` prompt로, `chatgpt-apps`는 planning prompt로 바뀐 공식 구조를 한국어 1:1 번역까지 동기화했습니다. local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 저장된 social research note를 baseline으로 다시 확인했습니다. 이번에는 Chrome plugin 연결에 성공했습니다. X targeted search는 readable body가 비어 있었고, Threads는 `DOMContentLoaded` 전에 timeout이 났으며, LinkedIn은 이미 공식 문서로 확인된 Goal Mode / Locked Computer Use 상태를 요약한 제3자 한국어 글만 보여주었습니다. 검증되지 않은 social-only claim은 공개 가이드에 추가하지 않았습니다.

### 누적 검증 메모

- 현재 `codex/models`를 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축으로 유지됩니다. `gpt-5.2`와 `gpt-5.3-codex`는 ChatGPT 로그인 Codex 기준 deprecated이므로 fallback처럼 쓰지 말고, 기존 script, config file, `codex exec --model` 명령이 이 모델명을 가리킨다면 추천 모델로 옮기는 편이 안전합니다. Codex product model picker, ChatGPT 일반 모델 retirement, API availability는 범위가 다를 수 있으므로 분리해서 읽어야 합니다. API-key authentication을 의도적으로 쓰는 workflow라면 API 모델 제공 여부를 따로 확인해야 합니다. 임의 provider로 Codex를 연결할 때 Chat Completions API 지원은 여전히 deprecated이며 future release에서 제거될 예정이므로, 새 설정은 Responses API 지원 provider를 기준으로 잡는 편이 안전합니다.
- 오늘 가장 중요한 변화는 pricing/API 범위 설명입니다. 현재 `codex/models` 문서는 `gpt-5.5`에 API Access를 표시하고 ChatGPT 또는 API-key authentication에서도 사용할 수 있다고 설명합니다. 동시에 현재 `codex/pricing`은 Plus, Pro 5x, Pro 20x, Business 사용량 표에는 GPT-5.5 local-message 범위를 보여주고, API Key 사용량 표도 GPT-5.5/GPT-5.4/GPT-5.4 mini를 usage-based로 표시합니다. 그래서 문서에서는 ChatGPT 로그인, API-key Codex, direct API 모델 호출을 분리해서 읽도록 정리했습니다. 직접 API에서 쓰는 `gpt-5.5`/`gpt-5.5-pro`와 예전 `gpt-5-codex` snapshot deprecation은 별도 OpenAI API 문서 범위로 분리했습니다.
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주되, IDE extension, CLI, Cloud도 같은 설치 탭 안에서 정식 경로로 유지합니다. `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `Enterprise & Edu`, `API Key` 기준의 사용량 표를 유지합니다. Plus 설명에는 최신 모델로 GPT-5.5, GPT-5.4, GPT-5.4 mini가 표시되고, Pro에는 GPT-5.3-Codex-Spark research preview가 별도 혜택으로 남아 있습니다. 가격 한도, 현재 추천 모델, API-key/direct API 가능 여부는 서로 다른 범위로 읽어야 합니다. Sites는 Business/Enterprise에서 available, Plus/Pro/API Key에서는 unavailable이며, Sites FAQ는 preview 기간 동안 무료라고 설명합니다. 2026년 6월 11-24일 eligible Plus/Pro 초대 reset banking은 dialog와 current terms에서 조건과 만료를 확인해야 합니다. ChatGPT for Excel 같은 agentic feature도 유효 시점 이후 Codex 사용 한도를 공유할 수 있습니다.
