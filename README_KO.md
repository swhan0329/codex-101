...
<p align="center">
  <strong>최종 업데이트: 2026년 5월 22일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-05-22)

- 2026년 5월 22일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex`는 cloud/code review 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축, `gpt-5.2`는 대표 대안 모델로 정리하는 편이 맞습니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 같이 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고 데스크톱에서는 App 경로를 우선 보여주며, `codex/pricing`은 `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, `API Key` 기준의 사용량 표를 유지합니다. Plus 기준 로컬 메시지 범위는 `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, `gpt-5.3-codex` 30-150입니다. Plus 설명에는 이제 web, CLI, IDE extension, iOS 접근도 함께 표시됩니다. 2026년 5월 22일 확인 기준으로 Pro $100은 2026년 5월 31일까지 2x 추가 사용량, Pro $200은 같은 날짜까지 usage boost가 적용됩니다.
...
- 공식 Codex [Follow goals](https://developers.openai.com/codex/use-cases/follow-goals) use case를 섹션 17에 반영했습니다. 문서에서는 `/goal`을 한 번의 프롬프트보다 길지만 무한 backlog는 아닌 작업을 위한 preview checkpoint contract로 다룹니다. 시작할 때 목표, 완료 조건, 검증 명령, 중간 checkpoint, 중단/승인 조건을 함께 고정하고, 상태 업데이트는 현재 checkpoint, 검증한 것, 남은 것, blocker로 짧게 받도록 안내합니다. CLI에서는 `[features] goals = true` 같은 feature flag가 운영 메모가 될 수 있으므로, App 전체에서 동일하게 지원된다고 단정하지 않습니다.
- `developers.openai.com/codex`, 관련 OpenAI 발표/검색 표면, API 모델 문서, Help Center의 remote access 문구, OpenAI API changelog도 폭넓게 다시 훑어봤지만, 오늘 기준으로 Codex 모델 추천 순서는 바뀌지 않았습니다. 대신 remote connection 설명을 connected host state, 같은 account/workspace, admin Remote Control/RBAC, SSH host 노출 주의, 공식 macOS setup 문구와 별도 Windows `remote_control` 운영 메모의 분리 기준으로 다시 다듬었습니다. Windows sandbox 엔지니어링 발표는 Windows/보안 섹션에 유지했고, 2026년 5월 18일 Dell Technologies 파트너십 발표는 일반 사용자가 오늘 켜는 기능이 아니라 enterprise/hybrid/on-prem 배포 맥락으로 분리했습니다. Chrome social verification에서는 X, Threads, LinkedIn에서 readable 결과를 확인했고, Windows feature flag로 remote control이 된다는 community claim도 보였습니다. 다만 공개 공식 문서와는 아직 상태가 엇갈리므로 일반 availability 문장으로 넣지 않고 운영 메모로만 유지합니다.
...
2026년 5월 22일 실행 참고: Chrome 연결은 성공했고 X, Threads, LinkedIn 모두에서 읽을 수 있는 검색 결과 일부를 확인했습니다. Windows `remote_control` flag 경로를 반복하는 community post가 있었지만, 공개 공식 문서는 아직 Windows phone 연결을 coming soon으로 설명하므로 일반 availability 문장이 아니라 운영 메모로만 유지했습니다.
