...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 13일</strong><br/>
...
- 공식 use-case catalog는 local 카드 60개와 공식 상세 페이지 접근 가능성을 기준으로 감사했습니다. 첫 카드는 현재 공식 Daily Work Brief 페이지를 유지했고, `promptEn`/`promptKo` 누락 0, multiline 한국어 요약 회귀 0입니다.
- Chrome으로 사람별 practitioner baseline을 X/LinkedIn에서 다시 대조하고 Threads 집계 검색도 별도로 확인했습니다. 새로 공개 가이드에 승격할 검증된 person-level Codex 팁은 없어서 공개 Section 20 practitioner cards는 유지하고, 로컬 social research note에 7월 13일 matrix를 업데이트했습니다.
...
- 2026년 7월 13일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. local 카드는 60개로 유지됐고 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 사람별 practitioner baseline을 다시 대조했습니다. X는 대부분 검색 shell/빈 본문, LinkedIn은 일부 프로필·검색 맥락, Threads는 집계 검색 결과 없음 수준이었고, 새로 공개 가이드에 승격할 검증된 person-level Codex 팁은 없었습니다. 공개 Section 20 practitioner cards는 유지하고 로컬 social research note에만 사람별 matrix를 업데이트했습니다.
