...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 3일</strong><br/>
...
- 공식 use-case catalog는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, trailing-space까지 포함한 official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 저장된 social research note를 baseline으로 다시 확인했지만, 이번 실행에서는 Chrome extension surface가 연결되지 않아 X/Threads/LinkedIn 최신 글을 안정적으로 읽지 못했습니다. 검증되지 않은 소셜 claim은 공개 가이드에 추가하지 않았습니다.
...
- 2026년 7월 3일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
