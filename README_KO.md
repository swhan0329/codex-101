...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 27일</strong><br/>
...
- openai.com/index 발표도 다시 대조했습니다. GPT-5.6 발표는 model/plan/pricing 맥락으로, NTT DATA와 OpenAI Presence 글은 enterprise adoption / agent-improvement 맥락으로, Health in ChatGPT는 Codex에서 아직 사용할 수 없다는 제품 경계로 분리했습니다. 모바일 원격 제어 발표는 현재 `remote-connections` 운영 문서와 계속 분리해 설명합니다.
- 사람별 공개 practitioner signal을 더 깊게 다시 확인해 이전 과소보고를 정정했습니다. Section 20에는 VB의 Auto-review/승인 운영, reasoning effort 조정, Gabriel Chua의 Build Week 초보자 흐름, Romain Huet의 GPT-5.6 모델 재점검 맥락을 사람별 카드로 추가했고, 제품 사실은 공식 문서 근거와 분리했습니다.
...
- 2026년 7월 27일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 99개로 유지됐고, 공식 Starter prompt 99개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
