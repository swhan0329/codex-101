...
<p align="center">
  <strong>최종 업데이트: 2026년 4월 14일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-04-14)

- `codex/models`를 다시 대조한 결과, 추천 모델은 오늘도 `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, `gpt-5.3-codex-spark`의 4개 축입니다. 달라진 점은 공개된 대안 모델 영역이 예전의 긴 GPT-5.x 목록보다 사실상 `gpt-5.2` 중심으로 정리되었다는 점입니다. 공식 문구에서는 여전히 `gpt-5.3-codex`의 코딩 역량이 GPT-5.4를 뒷받침한다고 설명합니다.
...
- 가격/접근 범위는 Quickstart와 Pricing을 함께 봐야 정확합니다. Quickstart는 모든 ChatGPT 플랜에 Codex가 포함된다고 설명하고, Pricing은 Business·Enterprise의 usage-based seat, 최대 $500 크레딧 프로모션, 토큰 기반 과금 전환, 그리고 Pro의 `$100` 10x / `$200` 20x 구간까지 함께 보여줍니다.
...
- Windows 문서는 이제 일반 Windows 가이드와 전용 Windows App 문서가 역할을 나눕니다. 기본 권장은 네이티브 앱이고, Microsoft Store나 `winget`로 설치할 수 있으며, `elevated` 샌드박스를 우선 쓰고 `unelevated`는 대안으로 봅니다. 전용 데스크톱 격리가 기본이고, IDE agent mode는 여전히 WSL 워크스페이스가 가장 안정적인 경로입니다.
...
- `config.toml` 최신화 포인트도 넓어졌습니다. 현재 reference는 `review_model`, top-level `web_search`, `tools.web_search`, `tools.view_image`, `personality`, `service_tier`, `default_permissions`, `windows.sandbox`, `windows.sandbox_private_desktop`, `model_instructions_file`, granular approval policy, app 권한 제어, feature flag, permissions profile까지 함께 다룹니다.
