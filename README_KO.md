...
<p align="center">
  <strong>최종 업데이트: 2026년 6월 16일</strong><br/>
...
### 일일 MCP 검증 스냅샷 (2026-06-16)

- `codex/models`를 가장 먼저 다시 확인했고, 추천 순서는 `gpt-5.5`, `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex-spark`로 유지됐습니다.
- 2026년 6월 12일 ChatGPT 릴리스 노트의 GPT-5.2 retirement를 추가로 반영했습니다. ChatGPT 일반 모델로서 GPT-5.2 Instant/Thinking/Pro가 더 이상 제공되지 않는다는 문구와, Codex models 문서의 `gpt-5.2`/`gpt-5.3-codex` ChatGPT-authenticated Codex deprecation 문구는 범위가 다르므로 섞어 쓰지 않습니다.
- Codex rate card는 대부분 고객에게 token-based pricing이 기본이고, GPT-5.5는 1M tokens 기준 input 125 credits, cached input 12.50 credits, output 750 credits로 표시됩니다. legacy average-message rate는 아직 migration 전인 일부 Enterprise 고객 범위로만 남겼습니다.
- 공식 use-case catalog는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, HTML entity normalization 후 official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
- 로그인된 Chrome으로 X, Threads, LinkedIn을 다시 확인했지만 X는 blank body, Threads는 search shell, LinkedIn은 navigation shell만 읽혀 새 social-only claim은 추가하지 않았습니다.

### 일일 MCP 검증 스냅샷 (2026-06-15)

- 2026년 6월 16일 기준으로 `codex/models`를 가장 먼저 다시 대조한 결과, `gpt-5.5`는 오늘도 최우선 추천 모델입니다. `gpt-5.4`는 주요 대체 선택지로 남고, `gpt-5.4-mini`는 빠른 로컬/subagent 축, `gpt-5.3-codex-spark`는 ChatGPT Pro용 거의 즉각적인 research preview 축으로 유지됩니다. 현재 models 문서는 계속 `gpt-5.2`와 `gpt-5.3-codex`를 ChatGPT 로그인으로 쓰는 Codex에서는 deprecated라고 명시합니다. ChatGPT 릴리스 노트는 별도로 2026년 6월 12일부터 GPT-5.2 Instant/Thinking/Pro가 ChatGPT에서 더 이상 제공되지 않는다고 설명하므로, Codex product deprecation, ChatGPT 일반 모델 retirement, API availability를 분리해서 읽어야 합니다. 아직 이 모델을 가리키는 script, config file, `codex exec --model` 명령은 추천 모델로 옮기되, API-key authentication을 의도적으로 쓰는 workflow라면 API 모델 제공 여부를 따로 확인해야 합니다. 임의 provider로 Codex를 연결할 때 Chat Completions API 지원은 여전히 deprecated이며 future release에서 제거될 예정이므로, 새 설정은 Responses API 지원 provider를 기준으로 잡는 편이 안전합니다.
...
- 2026년 6월 16일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, HTML entity normalization 후 official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
