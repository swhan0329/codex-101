...
<p align="center">
  <strong>최종 업데이트: 2026년 7월 22일</strong><br/>
...
- 최신 Codex changelog를 다시 확인했고 CLI 0.144.6를 추가했습니다. GPT-5.6 Sol/Terra/Luna bundled instructions가 새로고침됐고 context window가 272,000 tokens로 정정됐습니다. CLI 0.144.5의 dangerous-command detection 개선과 CLI 0.144.0-0.144.4 운영 메모는 계속 유지합니다.
...
- 2026년 7월 21일 ChatGPT for Small Business 프로그램은 ChatGPT Work adoption 맥락으로만 반영했습니다. product-specific training, 소상공인용 skills/plugins/partner workflow, Sites식 업무 예시는 유용하지만 Codex 모델 순서, 가격, remote-control, Windows 활성화, credential/permission 변화는 아닙니다.
- 2026년 7월 20일 long-horizon safety 글과 7월 21일 Hugging Face security incident 글은 persistent agent 보안 맥락으로 분리했습니다. 장시간 실행 모델은 sandbox 우회, trajectory-level monitoring, 사용자 visibility/control, pause/rollback 체계가 중요하다는 근거로만 반영하고, 일반 Codex 설정 변경처럼 단정하지 않았습니다.
- Chrome으로 X/LinkedIn 사람별 점검과 Threads aggregate 점검을 다시 진행했습니다. 공개 가이드에 올릴 만큼 검증된 새 person-level Codex tip은 없어서 Section 20 practitioner card는 유지했습니다.
...
- 2026년 7월 22일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 개별 use case는 99개로 유지됐고, 공식 Starter prompt 99개와 local `official.promptEn`의 정확 일치 mismatch는 0입니다. 첫 카드의 공식 URL과 metadata는 Daily Work Brief로 유지되며, `promptEn` 누락 0, `promptKo` 누락 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다.
