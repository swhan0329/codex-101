...
<p align="center">
  <strong>최종 업데이트: 2026년 6월 29일</strong><br/>
...
- `openai.com/index`에서 2026년 6월 26일 [Previewing GPT-5.6 Sol](https://openai.com/index/previewing-gpt-5-6-sol/)을 확인했습니다. 이는 select trusted partners/organizations 대상 preview이며 broader ChatGPT, Codex, API availability는 coming soon으로 설명되므로, 일반 Codex 모델 선택기 추천 순서나 일반 사용자 활성화 절차로 승격하지 않았습니다.
...
- 2026년 6월 25일 [How agents are transforming work](https://openai.com/index/how-agents-are-transforming-work/)는 agentic work 연구/노동시장 맥락으로만 반영했습니다. Codex-101이 비개발 직무와 knowledge workflow까지 다루는 이유를 설명하는 근거이지, 새 제품 surface나 활성화 절차 변화는 아닙니다.
...
- 저장된 social research note를 baseline으로 다시 확인했습니다. 이번 환경에서는 X/Threads/LinkedIn을 신뢰할 만큼 읽을 수 있는 Chrome 제어 경로가 충분하지 않아 공식 문서와 웹 확인을 우선했고, 새로 공식 문서로 대조 가능한 social-only Codex 팁은 추가하지 않았습니다.
...
- 2026년 6월 29일에는 공식 use-case catalog도 데이터셋 단위로 다시 감사했습니다. collection page를 제외한 공식 카드는 60개로 유지됐고, local/official ID 차이 0, `promptEn` 누락 0, `promptKo` 누락 0, official prompt mismatch 0, multiline 공식 prompt인데 한국어가 1줄 요약인 항목 0입니다. `slack-coding-tasks`와 `github-code-reviews`는 공식 페이지가 starter prompt를 open-in-Codex 버튼이 아니라 copy/export block에 싣는 예외로 계속 분리해 확인합니다.
