...
<p align="center">
  <strong>Last updated: May 13, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-05-13)

- Re-checking `codex/models` on May 13 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, and the API model docs did not change the recommended model order today, but it did require tightening the API-key availability wording. Chrome connected successfully, but active browser policy blocked Google, X, Threads, and LinkedIn, so no fresh social tips were added from those surfaces. Per the current page structure, daily review details stay in the bottom Changelog rather than a top update toggle.
...
Note for the May 13 run: Chrome connected successfully, but the active browser policy blocked Google, X, Threads, and LinkedIn access for this automation session. I kept the existing curated public sources and did not add fresh social claims that could not be verified in-browser.
