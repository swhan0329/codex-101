...
<p align="center">
  <strong>Last updated: May 22, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-05-22)

- Re-checking `codex/models` first on May 22 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
- Quickstart and Pricing now need to be read together. Quickstart still says every ChatGPT plan includes Codex and prefers the App path on desktop platforms, while `codex/pricing` lays usage out by `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, and `API Key`. On Plus, the published local-message ranges are `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, and `gpt-5.3-codex` 30-150. Plus now explicitly lists web, CLI, IDE extension, and iOS access. As of the May 22 check, Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 gets a usage boost through the same date.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, API model docs, Help Center remote-access notes, and the OpenAI API changelog did not change the recommended Codex model order today. It did require updating the remote-connection section around connected-host state, same account/workspace, admin Remote Control/RBAC, SSH host exposure, and the distinction between official macOS setup wording and the separate Windows `remote_control` operating memo. The Windows sandbox engineering post remains in Windows/security sections, and the May 18 Dell Technologies partnership stays enterprise/hybrid/on-prem deployment context rather than a feature general users can enable today. Chrome social verification found readable X, Threads, and LinkedIn results, including community claims that Windows remote control works with the feature flag. Those claims remain unverified against the public official docs, so the guide keeps them as an operating note rather than a general availability statement.
...
Note for the May 22 run: Chrome connected successfully and readable X, Threads, and LinkedIn results were available. Community posts repeated the Windows `remote_control` flag path, but public official docs still say Windows phone connection is coming soon, so the guide keeps that as an operating note rather than broad availability guidance.
