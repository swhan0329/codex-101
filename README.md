...
<p align="center">
  <strong>Last updated: May 16, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-05-16)

- Re-checking `codex/models` on May 16 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, API model docs, and the OpenAI API changelog did not change the recommended Codex model order today. It did require keeping API-key availability wording separate, adding the mobile preview announcement plus Windows `remote_control` enablement as Codex surface updates, and adding a Platform/API lane for May 2026 changelog items: the May 12 removal of DALL-E 2/3 snapshots and Realtime API Beta, Responses API web search `return_token_budget`, Realtime 2/Translate/Whisper, the OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, and Admin API SDK support. Chrome connected successfully through the Chrome plugin, but active browser policy blocked opening X, Threads, and LinkedIn, so no fresh social tips were added from those surfaces. Per the current page structure, daily review details stay in the bottom Changelog rather than a top update toggle.
...
Note for the May 16 run: Chrome connected successfully, but the active browser policy blocked opening X, Threads, and LinkedIn for this automation session. I kept the existing curated public sources and did not add fresh social claims that could not be verified in Chrome.
