...
<p align="center">
  <strong>Last updated: May 20, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-05-20)

- Re-checking `codex/models` first on May 20 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
- The main drift today remains authentication and API scope nuance. The current `codex/models` page marks `gpt-5.5` with API Access and says it is available in Codex with ChatGPT or API-key authentication, while `codex/pricing` still shows `gpt-5.5` as not available in the API Key usage-limit table. The guide treats this as a scope distinction to verify at point of use: ChatGPT sign-in remains the clearest Codex path for `gpt-5.5`, API users should confirm the actual model picker and standard API pricing, and direct API use of `gpt-5.5`, `gpt-5.5-pro`, and the Responses API-only `gpt-5-codex` belongs to the separate OpenAI API model docs.
- Quickstart and Pricing now need to be read together. Quickstart still says every ChatGPT plan includes Codex and prefers the App path on desktop platforms, while `codex/pricing` lays usage out by `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, and `API Key`. On Plus, the published local-message ranges are `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, and `gpt-5.3-codex` 30-150. Plus now explicitly lists web, CLI, IDE extension, and iOS access. As of the May 20 check, Pro $100 gets 2x extra usage through May 31, 2026, while Pro $200 gets a usage boost through the same date.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, API model docs, and the OpenAI API changelog did not change the recommended Codex model order today. It did require keeping official mobile-host wording separate from the Windows `remote_control` operating memo, keeping the Windows sandbox engineering post in Windows/security sections, and maintaining a Platform/API lane for May 2026 changelog/model items: the May 12 removal of DALL-E 2/3 snapshots and Realtime API Beta, Responses API web search `return_token_budget`, Realtime 2/Translate/Whisper, the OpenAI Developers plugin for Codex, TypeScript Agents SDK, `chat-latest`, Admin API SDK support, and the Responses API-only `gpt-5-codex` model. Chrome connected successfully through the Chrome plugin and readable X, Threads, and LinkedIn results were checked. Unverified social claims were not added; the only usable takeaways were already aligned with official guidance: keep permissions explicit, define output contracts, write tool-use rules, set completion criteria, and let Codex invoke the OpenAI Developers plugin when API/app work calls for it. Per the current page structure, daily review details stay in the bottom Changelog rather than a top update toggle.
...
Note for the May 20 run: Chrome connected successfully and readable X, Threads, and LinkedIn results were available. I did not add unverified claims; I only retained official-doc-aligned reminders around permissions, output contracts, tool-use rules, completion criteria, and the OpenAI Developers plugin.
