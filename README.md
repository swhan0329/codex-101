...
- Rechecked What's new and the current Codex changelog. CLI 0.149.0 adds an interactive `codex agents` task dashboard, `/cd`, `/pwd`, and `/cwd` TUI commands, and `codex queue` for messaging existing sessions. `codex doctor` now covers endpoint protection, network/proxy failures, desktop-app state, and update connectivity; resumed or forked threads also restore their active permission profile. The prior 0.148.0 export, fork, archive/restore, and Hooks updates remain available.
