...
<p align="center">
  <strong>Last updated: June 6, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-06-06)

- Re-checking `codex/models` first on June 6 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, and `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro. The important change is that the current models page now lists `gpt-5.2` and `gpt-5.3-codex` as deprecated in Codex when you sign in with ChatGPT; scripts, config files, or `codex exec --model` commands that still reference them should move to the recommended models unless the workflow intentionally uses API-key authentication and confirms API availability. If you point Codex at arbitrary providers, the Chat Completions API support path is still documented as deprecated and scheduled for removal in a future Codex release, so new provider setup should prefer Responses API support.
...
- Quickstart and Pricing still need to be read together. Quickstart says every ChatGPT plan includes Codex and prefers the App path on desktop platforms, while `codex/pricing` lays usage out by `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, and `API Key`. On Plus, the published local-message ranges still include `gpt-5.3-codex` 30-150 even though the models page now deprecates that model for ChatGPT sign-in, so the guide treats pricing limits and current model recommendations as separate scopes. The old Pro promo wording is no longer in the current pricing docs; Pro is now described as 5x or 20x higher rate limits than Plus. Plus explicitly lists web, CLI, IDE extension, and iOS access. The pricing matrix also lists Sites as available for Business and Enterprise, unavailable for Plus, Pro, and API Key, and the Sites FAQ says pricing is free while in preview. Agentic features such as ChatGPT for Excel can share Codex usage limits once their pricing is effective.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, API model docs, Help Center release notes, and the OpenAI API changelog changed the deprecated-model interpretation but did not change the top recommended model order. It keeps the June 2 [Codex for every role, tool, and workflow](https://openai.com/index/codex-for-every-role-tool-workflow/) announcement plus the official Sites docs as a workflow expansion, keeps the June 2 knowledge-work report and May 13 macOS certificate-update security note, and keeps the June 1 OpenAI/AWS Bedrock update and May 29 Windows/Profile release notes in their own lanes. Chrome social verification produced no substantive new content this run: X rendered blank, Threads timed out, and LinkedIn only exposed navigation chrome. No social-only claim changed the guide.
...
Note for the June 6 run: keep deprecated Codex model status separate from API model availability. `gpt-5.2` and `gpt-5.3-codex` are deprecated for ChatGPT-authenticated Codex, while some deprecated-for-ChatGPT models may still be available through API-key authentication or direct API use.
