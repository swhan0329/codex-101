...
<p align="center">
  <strong>Last updated: April 9, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-04-09)

- Re-checking `codex/models` still shows the same four recommended lanes: `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark`. The important change versus older drafts is that the alternative-model surface is now effectively just `gpt-5.2`, not the longer legacy list this guide used before. The most important official wording to preserve is still that the coding capabilities of `gpt-5.3-codex` now power GPT-5.4.
...
- Quickstart and Pricing changed materially since the last draft. Quickstart still says every ChatGPT plan includes Codex, and the April 2, 2026 OpenAI pricing announcement adds the team lens: Business and Enterprise can add Codex-only pay-as-you-go seats with no fixed seat fee, plus eligible Business workspaces can earn up to $500 in credits. The pricing page now also distinguishes which workspaces already use token-based rate cards: new and existing Business plus new Enterprise are on the new token table, while other plans still show legacy message-based averages for now.
- The setup story is clearer than older revisions: Quickstart treats the app as the easiest starting path, the IDE surface explicitly covers VS Code, Cursor, Windsurf, VS Code Insiders, and JetBrains, and API-key sign-in is still allowed but can limit features such as cloud threads.
...
- The Codex app docs are broader than a simple desktop shell. The current app, app/features, and app/windows pages emphasize built-in worktrees, Automations, Git tools, integrated terminal, voice dictation, pop-out windows, IDE sync, image input, notifications, sleep prevention, Windows-native agent vs WSL agent switching, and terminal/editor preferences, so the guide now surfaces that operating model earlier.
...
- `config.toml` is still the area most likely to drift. The current reference gives more weight to `review_model`, top-level `web_search`, `tools.web_search`, `personality`, `service_tier`, `default_permissions`, `tools.view_image`, `windows.sandbox`, `windows.sandbox_private_desktop`, `model_instructions_file`, granular approvals, app permissions, feature flags, named permission profiles, and `requirements.toml` constraints for managed environments.
