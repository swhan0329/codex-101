...
<p align="center">
  <strong>Last updated: August 13, 2026</strong><br/>
...
- Re-audited the official use-case catalog: official individual pages 101 after excluding collection pages, local cards 101, `promptEn` gaps 0, `promptKo` gaps 0, official Starter prompt mismatches 0 after updating `deep-security-scan`, and line/token structure regressions 0.
...
- Rechecked openai.com/index and OpenAI Developers/API docs. No Codex product model recommendation change was needed. The August 12 RingCentral story and August 12 enterprise research post are now treated as enterprise adoption context: Codex and ChatGPT Work are being used for end-to-end internal builds, PMO status/reporting/release operations, cross-functional workflow packaging, and broader legal/sales/recruiting/marketing agentic work. They do not introduce a new model, feature flag, plan, credential change, or setup step. The August 11 Daybreak on AWS post remains security deployment context for approved Daybreak Access customers, and the August 10 Daybreak posts remain approved-defender security context rather than general Codex model guidance. Linux is now split between the Codex CLI path and the ChatGPT desktop app for Linux preview: Ubuntu 24.04/26.04, Debian 13, and Fedora 43/44 are supported on x64/ARM64 through `.deb` or `.rpm` packages, while Computer Use is still not available in the Linux preview.
- Person-level practitioner signals were rechecked in Chrome against X, LinkedIn, and Threads. A new Linux desktop preview practitioner card was promoted because VB and Dominik's current posts are corroborated by the official Linux app docs; the card keeps the preview and Computer Use limits visible.
