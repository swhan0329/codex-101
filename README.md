...
- Rechecked What's new and the current Codex changelog. CLI 0.148.0 adds TUI Markdown export with `/export`, `codex exec fork`, archive/restore from the TUI resume picker, draft prompts during startup, estimated thread credits or cost in `/status` and terminal surfaces for eligible workspaces, built-in Amazon Bedrock Runtime routing, and hooks that can run asynchronously or invoke MCP tools. It also fixes stale model instructions during model switches, restores persisted cwd and approval policy on resume, improves MCP OAuth recovery, and tightens fail-closed sandbox behavior across Linux and Windows.
- Added the Apple Messages plugin boundary from the August 17–21 update. It runs only in Codex or ChatGPT Work in the Apple Silicon ChatGPT desktop app; grant the requested macOS permissions, review the message and recipients, then approve the send. Keep **Ask for approval** or **Approve for me** when sending, because Full access can prevent the required confirmation.
