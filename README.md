...
<p align="center">
  <strong>Last updated: April 6, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-04-06)

- Re-checking `codex/models` still shows the same four recommended lanes: `gpt-5.4`, `gpt-5.4-mini`, `gpt-5.3-codex`, and `gpt-5.3-codex-spark`. The practical distinction to preserve is that `gpt-5.4` and `gpt-5.4-mini` are the default local-first paths, while `gpt-5.3-codex` is still the only recommended lane with Codex Cloud enabled.
...
- Quickstart currently says every ChatGPT plan includes Codex. Pricing now needs to be read in two layers: individuals look at Free/Go/Plus/Pro usage bands, while teams should also account for Business and Enterprise usage-based seats, credit promotions, and the ongoing move toward token-based billing.
...
- Windows guidance is now split more cleanly between the general Windows page and the dedicated Windows app page. The default recommendation is the native app, installable from Microsoft Store or `winget`, with `elevated` sandbox preferred over `unelevated`, private desktop isolation on by default, and WSL positioned as the right option when your workflow already lives in Linux or when IDE agent mode on Windows needs the most reliable path.
- The Codex app docs are broader than a simple desktop shell. The current app/features page emphasizes built-in worktrees, Automations, Git tools, an integrated terminal, voice dictation, pop-out windows, IDE sync, image input, notifications, and sleep prevention, so the guide now surfaces that operating model earlier.
- Plugins are now documented as a real packaging layer. The official `Plugins` page clearly defines plugins as bundles of skills, app integrations, and MCP servers, and documents both the app plugin directory and the CLI `/plugins` flow.
- `config.toml` is still the area most likely to drift. The current config basics page makes the precedence rules, `web_search = "cached"` default, `personality`, `shell_environment_policy`, feature flags, `windows.sandbox`, `windows.sandbox_private_desktop`, and granular approvals worth surfacing more clearly.
