...
<p align="center">
  <strong>Last updated: August 7, 2026</strong><br/>
...
- Rechecked What's new and the current Codex changelog. CLI 0.146.0 adds named chats, pinned threads, temporary forks, standalone web search for compatible custom providers, executor-provided skills, Agent Plugin manifest support, workspace plugin publishing, and app-server/WebSocket improvements. The desktop app also now surfaces Activity view, browser-history/open-tab context, multi-repo review summaries, and image viewer/canvas refinement.
- Rechecked the official use-case catalog. It still has 101 detail pages and all 101 local `official.promptEn` values match the official `starterPrompt.body` exactly; the first card's canonical detail slug is now `daily-work-brief`, with `proactive-teammate` retained as a local hash alias.
...
- Rechecked openai.com/index and OpenAI Developers/API docs. No Codex product model recommendation change was needed, but the August 4 education announcement introduces K-12 Educator, College Educator, and College Student plugins for ChatGPT Work and Codex. The August 6 GPT-5.6 Sol ChatGPT announcement is treated as ChatGPT model-access context only because it explicitly leaves the Work/Codex Sol variant unchanged.
- Person-level practitioner signals were rechecked in Chrome against X, LinkedIn, and Threads. Section 20 now refreshes Ryan Lopopolo's skills guidance: use skills as runbooks that shape Codex's working theory, not deterministic programs. A previously staged Derrick model-selection card was not promoted because the current in-window social evidence did not verify the linked claim.
