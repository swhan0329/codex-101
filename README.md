...
<p align="center">
  <strong>Last updated: June 1, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-06-01)

- Re-checking `codex/models` first on June 1 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
- Quickstart and Pricing now need to be read together. Quickstart still says every ChatGPT plan includes Codex and prefers the App path on desktop platforms, while `codex/pricing` lays usage out by `Free`, `Go`, `Plus`, `Pro 5x`, `Pro 20x`, `Business`, and `API Key`. On Plus, the published local-message ranges are `gpt-5.5` 15-80, `gpt-5.4` 20-100, `gpt-5.4-mini` 60-350, and `gpt-5.3-codex` 30-150. Plus now explicitly lists web, CLI, IDE extension, and iOS access. As of the June 1 check, the official docs and Help Center still describe the Pro $100 2x promo and Pro $200 higher 25x five-hour Codex limits as running through May 31, 2026. Because that end date has now passed, users should verify current limits in the Codex usage dashboard and plan banner.
...
- A wider sweep across `developers.openai.com/codex`, related OpenAI announcement/search surfaces, API model docs, Help Center release notes, and the OpenAI API changelog did not change the recommended Codex model order today. It keeps the May 29 release-note wording for Windows Computer Use, Windows-host remote control, responsiveness/browser stability improvements, and Codex Profiles; it also keeps the May 21 notes for Appshots, Goal mode GA, locked computer use, in-app browser annotations, admin Codex analytics, and plugin sharing defaults. The May 29 OpenAI customer story [How Braintrust turns customer requests into code with Codex](https://openai.com/index/braintrust/) and official resource [How OpenAI uses Codex](https://openai.com/business/guides-and-resources/how-openai-uses-codex/) are reflected as workflow patterns: turn customer requests into preview branches, start large changes in Ask Mode, write prompts like issues, improve the environment, use AGENTS.md, and compare alternatives with Best-of-N. The May 27 OpenAI engineering post, [Building self-improving tax agents with Codex](https://openai.com/index/building-self-improving-tax-agents-with-codex/), remains an advanced operating pattern rather than a new toggle. Chrome social verification could read X, Threads, and LinkedIn, but X included unverified usage-reset claims, so only official-doc-corroborated patterns were added.
...
Note for the June 1 run: the official docs and release notes now say Windows hosts can be controlled from ChatGPT mobile or a Mac, while the Windows Codex App still cannot control another computer. Windows Computer Use is newly documented for eligible users, but it is region-gated at launch and should not be described as universally available.
