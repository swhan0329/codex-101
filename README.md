...
<p align="center">
  <strong>Last updated: August 27, 2026</strong><br/>
...
- Rechecked What's new and the current [Codex changelog](https://learn.chatgpt.com/docs/changelog). CLI 0.150.0 adds `@` references to other Codex tasks from the terminal, a `/copy` picker for whole responses, code blocks, or blockquotes, descriptive titles for unnamed tasks plus `/rename` suggestions, terminal link labels, permission-mode shortcut bindings, and `Interrupt` hooks. Treat the security fixes as an operating boundary: an untrusted project cannot provide project-level `AGENTS.md`, and managed deny-read rules remain enforced after a permission change. CLI 0.150.1 fixes remote compaction so retained images count toward the token budget by default.
