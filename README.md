...
<p align="center">
  <strong>Last updated: May 18, 2026</strong><br/>
...
### Daily MCP Verification Snapshot (2026-05-18)

- Re-checking `codex/models` on May 18 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
Note for the May 18 run: Chrome connected successfully. X returned blank readable content, while Threads and LinkedIn exposed only search/navigation chrome without usable current Codex posts, so I kept the existing curated public sources and did not add fresh social claims.
