...
### Daily MCP Verification Snapshot (2026-05-17)

- Re-checking `codex/models` on May 17 confirms that `gpt-5.5` is still the top recommended Codex model when it appears in your picker. `gpt-5.4` remains the main fallback, `gpt-5.4-mini` stays the fast local/subagent lane, `gpt-5.3-codex` remains the cloud/code-review lane, `gpt-5.3-codex-spark` stays the near-instant research-preview lane for ChatGPT Pro, and `gpt-5.2` is still the main alternative model.
...
Note for the May 17 run: Chrome connected successfully, but the active browser policy blocked opening X, Threads, and LinkedIn for this automation session. I kept the existing curated public sources and did not add fresh social claims that could not be verified in Chrome.
